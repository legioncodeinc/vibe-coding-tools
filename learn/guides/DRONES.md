...
The [Git Drone](../../plugins/wasp-nest-core/agents/git-wasp-drone.md), for example, owns history, recovery, conflicts, and worktrees. It does not own CI pipelines or credential rotation. Its [Git Stinger](../../plugins/wasp-nest-core/skills/git-stinger/SKILL.md) gives the recovery and safety procedures. That positive-and-negative description is more useful than a vague "Git expert" label.
...
The public marketplace cannot be edited in place; it is generated. For a new domain, first check the [catalog](../reference/PLUGIN-CATALOG.md) and [Pest Controller roster](../../plugins/wasp-nest-core/skills/pest-controller-suit/SKILL.md). The maintainers use the Queen Wasp `/forge` workflow to define, research, and validate a new component, then `/register` to pair and route it. [Open a public issue](https://github.com/legioncodeinc/vibe-coding-tools/issues) with the missing task and the nearest existing Drone if you want one added.
