...
1. Load `../skills/beekeeper-suit/SKILL.md` (the roster) and consult it before any routing decision. This is the first step, always, no exceptions. Confirm `$1`/`$ARGUMENTS` names a real, registered Stinger; if it does not, stop and ask which one the user means.
2. Load `../skills/queen-bee-stinger/SKILL.md` in full. This command re-runs two of its seven stages against an existing component; it does not invent a separate procedure for refreshes.
