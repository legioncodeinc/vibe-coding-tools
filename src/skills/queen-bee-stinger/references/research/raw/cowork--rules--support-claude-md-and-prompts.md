...
| `<project-root>/CLAUDE.md` | This project | Architecture, conventions, and commands. This is the main one. |
| `<subdirectory>/CLAUDE.md` | That subdirectory (loaded on demand when Claude reads files in that directory, not at session start) | Module-specific rules (for example, different conventions in `frontend/` vs `api/`). |
...
## NOTE ON COWORK APPLICABILITY

This article is written primarily for Claude Code (the CLI). Cowork's equivalent "rules" mechanism is different: Cowork does not read a project-root CLAUDE.md file from disk in the same way. Instead, Cowork exposes "Global instructions" (Settings > Cowork > Global instructions) that apply to every Cowork session, and "Folder instructions" that add project-specific context when a local folder is selected on desktop (Claude can also update folder instructions itself during a session). See the companion file `cowork--agents--support-get-started-cowork.md` in this archive for the primary-source description of that mechanism. Cowork sessions run in cloud/VM sandboxes (`/sessions/.../mnt/...`) where a `.claude/CLAUDE.md` style file is not the standard mechanism; skills and plugins are the dominant persistent-instruction unit in Cowork.
