...
**Recommendation: `creds_key`.** A named credential reference rotates without touching the dataset URI, and the secret never ships in app config. Raw creds here would be a should-refactor finding (`guides/deeplake-08-storage-backends.md`). The actual secret storage, scope, and rotation cadence are `security-worker-bee`'s audit.
