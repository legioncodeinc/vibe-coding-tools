# Open Questions - vector-store-stinger
...
2. **Should the memory-table BM25 oid bug be tracked upstream?** BM25 (`deeplake_index`) is disabled on the memory table today. If Activeloop fixes the oid bug, `guides/deeplake-02-indexing.md` can re-enable standalone BM25 on memory.
...
1. **Deep Lake SQL surface additions** - new operators or index types beyond `deeplake_index` / `<#>` / `deeplake_hybrid_record`. Revisit `guides/deeplake-02-indexing.md`.
2. **BYOC backend additions** - new storage schemes or credential models beyond `creds_key`. Revisit `guides/deeplake-08-storage-backends.md`.
3. **DeeplakeApi tuning** - whether `MAX_RETRIES=3` / `MAX_CONCURRENCY=5` change as load grows. Revisit `guides/deeplake-05-querying-deeplakeapi.md`.
