# Research Plan - vector-store-stinger

**Bee:** vector-store-worker-bee
**Forged:** 2026-06-16
...
1. Should vector-store-worker-bee own retrieval / RAG? **Resolved: NO.** It picks the `FLOAT4[]` shape and the search operator (`<#>` / hybrid), then hands chunking / reranking / eval to `retrieval-worker-bee`. Noted in `guides/deeplake-00-principles.md`.
...
3. Where does dataset versioning live vs the per-row version-bump? **Resolved:** dataset commit/branch/tag/revert_to in `guides/deeplake-04-versioning-branches.md`; the append-only per-row version-bump in `guides/deeplake-06-embeddings-jsonb-versioning.md`. Cross-referenced so they are never conflated.
