...
Three intertwined storage decisions: how embeddings are stored (`FLOAT4[]`), how schemaless payloads are stored (JSONB `message`), and how edits are recorded (append-only version-bump). vector-store-worker-bee owns the storage shape; retrieval is `retrieval-worker-bee`.
...
`<#>` cosine distance, optionally combined with BM25 via hybrid. The operator choice is `guides/02-indexing.md`. vector-store-worker-bee picks the column shape and the operator, then hands chunking / reranking / eval to `retrieval-worker-bee`.
