...
Deep Lake datasets are bring-your-own-cloud. The same dataset can live on Activeloop-managed storage, your own object store, or local / in-memory. vector-store-worker-bee picks the backend and the credential model; `security-worker-bee` audits the secrets.
