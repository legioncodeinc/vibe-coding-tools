...
**Date:** {{YYYY-MM-DD}}
**Reviewer:** vector-store-wasp-drone
...
- [ ] `library-wasp-drone` - {{if any schema-PRD updates needed}}
- [ ] `typescript-node-wasp-drone` - {{if any read-amplification risks at the TypeScript data-access edge}}
- [ ] `security-wasp-drone` - {{if any creds / creds_key / token / PII findings}}
- [ ] `retrieval-wasp-drone` - {{if any embedding-storage / retrieval decisions}}
- [ ] `quality-wasp-drone` - {{post-heal verification queries}}
...
*Produced by vector-store-stinger. See `SKILL.md` for methodology.*
