...
## Verification queries (handed to `quality-wasp-drone`)
...
- For whole-dataset recovery from a bad bulk write, `revert_to` a prior commit or tag (`guides/deeplake-04-versioning-branches.md`).

## Handoffs

- **`quality-wasp-drone`** - runs the verification queries above; confirms green.
- **`security-wasp-drone`** - review any new PII column.
- **`typescript-node-wasp-drone`** - flag any data-access changes the TypeScript call sites must adopt.

## References

- `guides/deeplake-03-schema-healing.md`
- `guides/deeplake-01-schema-design.md` SSNOT-NULL-DEFAULT
- {{external Deep Lake / Activeloop docs URLs}}

---

*Template from `vector-store-stinger/templates/migration-plan.md`. See `examples/schema-heal-add-column.md` for a filled example.*
