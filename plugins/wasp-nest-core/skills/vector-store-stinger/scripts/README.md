...
Diff `desired = ColumnDef[].map(c => c.name)` minus `live`. Anything in `desired` but not `live` is a missing column the heal would add (additively, never `IF NOT EXISTS`). Source: `guides/deeplake-03-schema-healing.md`.

## NOT-NULL-needs-DEFAULT audit

`validateSchema()` enforces that every NOT NULL column carries a DEFAULT. Audit the ColumnDef list in code: any `{ notNull: true }` with no `default` is a must-fix before any heal. Source: `guides/deeplake-01-schema-design.md`.
...
Source: `guides/deeplake-02-indexing.md`.
...
Source: `guides/deeplake-06-embeddings-jsonb-versioning.md`.
