...
Deep Lake datasets are bring-your-own-cloud. The same dataset can live on Activeloop-managed storage, your own object store, or local / in-memory. vector-store-wasp-drone picks the backend and the credential model; `security-wasp-drone` audits the secrets.
...
For storage-choice invocations, fill in `examples/storage-backend-choice-walkthrough.md` with the deployment mapped to the matrix above. Recommend a primary backend and credential model with 2-3 cited reasons, and state what `security-wasp-drone` should audit on the credentials.
...
- Hand creds / `creds_key` audit to `security-wasp-drone`.
