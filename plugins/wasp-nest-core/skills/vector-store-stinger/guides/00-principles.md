...
- **Which embedding model, and its dimension** -> `embeddings-runtime-wasp-drone`. This Stinger picks the column/collection shape and the index; the model that produces the vectors is theirs.
- **Recall tuning, hybrid weighting, chunking strategy, evaluation** -> `retrieval-wasp-drone`. This Stinger stores and indexes; retrieval-wasp-drone tunes how well a query finds the right rows.
- **Tenant isolation, credential handling, PII in stored payloads** -> `security-wasp-drone`.
- **Schema PRD authorship** -> `library-wasp-drone`.
