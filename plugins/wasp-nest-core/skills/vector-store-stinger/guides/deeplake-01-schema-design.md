...
Mark columns that hold PII so `security-wasp-drone` can audit creds, encryption, and retention. Note the PII columns in the schema spec and surface them in any review with file:line. vector-store-wasp-drone flags PII; security-wasp-drone audits the controls (including `creds_key` and BYOC credentials).
