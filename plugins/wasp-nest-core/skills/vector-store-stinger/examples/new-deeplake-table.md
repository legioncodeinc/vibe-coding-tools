...
A clean new Deep Lake table for Hivemind: the `codebase` table that holds indexed code chunks, single-sourced as a `ColumnDef[]`, created `USING deeplake`, with an embedding tensor, a JSONB payload, and a vector + hybrid search plan. Source: `guides/deeplake-01-schema-design.md`, `guides/deeplake-02-indexing.md`, `guides/deeplake-06-embeddings-jsonb-versioning.md`.
...
| Decision | Source |
|---|---|
| One `ColumnDef[]` is the single source | `guides/deeplake-07-no-orm-columndef.md` |
| `USING deeplake` on CREATE TABLE | `guides/deeplake-07-no-orm-columndef.md` SSbuildCreateTableSql |
| Every NOT NULL column carries a DEFAULT | `guides/deeplake-01-schema-design.md` SSNOT-NULL-DEFAULT - `validateSchema()` gate |
| `metadata` is JSONB (schemaless per-chunk) | `guides/deeplake-06-embeddings-jsonb-versioning.md` SSjsonb |
| `embedding` is `EMBEDDING` -> `FLOAT4[]` 768-dim | `guides/deeplake-06-embeddings-jsonb-versioning.md` SSembeddings |
| Lookup index on `repo` via `ensureLookupIndex` | `guides/deeplake-02-indexing.md` SSlookup - hot equality, marker-cached |
| BM25 `deeplake_index` allowed (not memory) | `guides/deeplake-02-indexing.md` SSbm25 |
| `<#>` cosine on `FLOAT4[]` | `guides/deeplake-02-indexing.md` SSvector |
| Hybrid 0.7 / 0.3 weighting | `guides/deeplake-02-indexing.md` SShybrid + `research/2026-06-16-deeplake-search-hybrid-weighting.md` |
| `version` column for append-only edits | `guides/deeplake-06-embeddings-jsonb-versioning.md` SSversion-bump |
...
- [ ] Search operators chosen per `guides/deeplake-02-indexing.md` (lookup / BM25 / vector / hybrid).
- [ ] Storage backend and credential model picked (`guides/deeplake-08-storage-backends.md`).

## Handoffs from this table

- `security-wasp-drone` - audit the BYOC creds / `creds_key` if not on `al://`.
- `typescript-node-wasp-drone` - TypeScript data-access plan for surfacing search results.
- `retrieval-wasp-drone` - retrieval over `codebase.embedding` (chunking, top-k, reranking).
- `quality-wasp-drone` - verification queries after first ingest.

---

*Source: `guides/deeplake-01-schema-design.md`, `guides/deeplake-02-indexing.md`, `guides/deeplake-06-embeddings-jsonb-versioning.md`. Forged 2026-06-16.*
