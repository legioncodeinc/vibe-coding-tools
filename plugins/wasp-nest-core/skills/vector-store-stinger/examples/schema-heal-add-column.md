...
Add a NOT NULL `summary_embedding` column to the live `skills` table via `healMissingColumns()` - additively, with a DEFAULT, never `IF NOT EXISTS`. Source: `guides/deeplake-03-schema-healing.md`, `guides/deeplake-01-schema-design.md`, `templates/migration-plan.md`.
...
- **Naive single-step that would fail:**
  ...
  ```
  Deep Lake returns HTTP 500 (not 409) on a duplicate add, so `IF NOT EXISTS` is no guard, and the DeeplakeApi retry layer would retry the 500 three times and still fail. The guard is the diff, not `IF NOT EXISTS`. Source: `guides/deeplake-03-schema-healing.md` SS500-not-409.
...
## Step 4 - verification (handed to `quality-wasp-drone`)
...
There is no destructive rollback path in a heal - the heal only adds. If the column was a mistake, remove it from the ColumnDef list (so future heals stop expecting it) and, if it must come off the live table, do that as a deliberate, separately-reviewed change. For dataset-level recovery from a bad bulk write, `revert_to` a prior commit (`guides/deeplake-04-versioning-branches.md`).
...
- `guides/deeplake-03-schema-healing.md` SS500-not-409, SSvalidateSchema.
- `guides/deeplake-01-schema-design.md` SSNOT-NULL-DEFAULT.
