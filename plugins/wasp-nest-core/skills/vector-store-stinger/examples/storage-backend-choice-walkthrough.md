...
A full storage-backend choice walkthrough for a Hivemind deployment. Source: `guides/deeplake-08-storage-backends.md`, `research/2026-06-16-storage-backends-creds.md`.
...
From `guides/deeplake-08-storage-backends.md`:
...
**Recommendation: `creds_key`.** A named credential reference rotates without touching the dataset URI, and the secret never ships in app config. Raw creds here would be a should-refactor finding (`guides/deeplake-08-storage-backends.md`). The actual secret storage, scope, and rotation cadence are `security-wasp-drone`'s audit.
...
## What `security-wasp-drone` should audit
...
- `guides/deeplake-08-storage-backends.md`.
