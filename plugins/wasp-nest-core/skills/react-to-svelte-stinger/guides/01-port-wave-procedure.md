...
5. **Ship Gate.** Security, quality, and repo-health passes per the paired drone's reporting expectations before the wave is called done.
...
Waves parallelize across drones only after the shared shell and contract inventory exist. Safe parallel split: one drone per surface, each owning disjoint route directories. Shared files (data layer, design tokens, shared components) are single-owner: the orchestrator assigns them to one drone per wave, others consume.
