...
This folder accumulates durable recommendation and audit reports produced by `embeddings-runtime-wasp-drone`.

## Report types

- **Provider/runtime selection reports** - dated recommendations on hosted provider vs self-hosted (see `guides/00-selection-matrix.md`), or on whether to enable embeddings for a given workload at all (see `guides/local-daemon-05-embeddings-vs-bm25.md` for the Wasp Nestmind-specific worked version of that question).
- **Model swap reports** - recorded model or provider swap decisions following `templates/embedding-model-swap-plan.md`.
- **Dim migration reports** - completed dimension-change records following `templates/dim-migration-checklist.md`, noting the schema-heal or column-migration handoff to `vector-store-wasp-drone`.
