# Local-daemon: local vs hosted inference, the Wasp Nestmind case

*This is the Wasp Nestmind-specific worked example. For the general, stack-neutral version of this decision (which this repo's Drone applies by default), see `00-selection-matrix.md`.*
...
| Key management | None | API key handling (hand to security-wasp-drone) |
...
2. **Security handoff.** API key storage and the data-egress review belong to security-wasp-drone. This stinger weighs the tradeoff; it does not own the key or the egress sign-off.
