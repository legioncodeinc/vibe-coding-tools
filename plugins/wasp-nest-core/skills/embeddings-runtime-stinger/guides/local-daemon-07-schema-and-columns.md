...
1. **Column resize.** The `FLOAT4[]` columns must be re-sized to the new width. This is the schema-heal path on the Deep Lake dataset, owned by vector-store-wasp-drone.
...
2. **If a schema event:** write the swap plan (`templates/embedding-model-swap-plan.md`) and the checklist (`templates/dim-migration-checklist.md`), then hand the column resize to vector-store-wasp-drone.
3. **Re-embed and validate** before declaring the migration complete.

## Handoff

The schema-heal mechanics, how the Deep Lake dataset actually resizes a `FLOAT4[]` column and reconciles existing rows, belong to vector-store-wasp-drone. This stinger decides the dimension, writes the plan, and triggers the handoff. It does not execute the schema event itself.
