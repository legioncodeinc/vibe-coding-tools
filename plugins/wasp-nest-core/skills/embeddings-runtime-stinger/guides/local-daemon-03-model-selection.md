...
*This guide is the worked example of local-model selection for the Wasp Nestmind product specifically (nomic-embed-text-v1.5 and its direct 768-dim alternatives). For general, stack-neutral model selection across hosted providers and local options, start at `00-selection-matrix.md`.*
...
- The mechanics of resizing the columns; that is the schema-heal path, owned by vector-store-wasp-drone.
