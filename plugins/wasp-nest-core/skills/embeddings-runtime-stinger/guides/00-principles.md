...
- Hand the schema execution to the owning storage Drone: `vector-store-wasp-drone` on this repo's default stack (pgvector on Neon) or on a Deep Lake project. This stinger decides the dimension and writes the plan; it does not execute the schema event itself.
