...
A filled-in comparison applying the Wasp Nestmind-scoped rubric from `guides/local-daemon-03-model-selection.md`. The question is narrow: is any candidate worth swapping in for `nomic-embed-text-v1.5` (q8, 768 dim) on Hivemind's recall?
...
Use `templates/embedding-model-swap-plan.md`. If the chosen model is not 768 dim, also run `templates/dim-migration-checklist.md` and hand the column resize to vector-store-wasp-drone.
