...
description: "Plan search-led blog content. Use for topic clusters, titles, post structure, cadence, AEO formatting, or calls to action. Read README.md for the guide map."
...
- Technical SEO implementation (schema markup, robots.txt, Core Web Vitals, hreflang) → `seo-aeo-wasp-drone`
- CMS setup or hosting → `website-wasp-drone`
...
- **Hand off technical SEO to `seo-aeo-wasp-drone`.** This Stinger owns strategy and copy; schema, sitemap, and Core Web Vitals are out of scope.
...
- **Cluster map** → hand to `seo-aeo-wasp-drone` for schema markup and technical implementation of the pillar page architecture.
...
*Stinger forged by `stinger-forge` from `blogging-content-strategy-wasp-drone-command-brief.md` and research gathered by `scripture-historian`. Part of the Legion AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
