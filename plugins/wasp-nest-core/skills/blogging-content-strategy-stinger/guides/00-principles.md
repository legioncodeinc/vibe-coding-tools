...
### 6. Hand off technical SEO decisions to `seo-aeo-wasp-drone`

**Why:** This Stinger owns strategy and copy. Implementation details — robots meta tags, hreflang, sitemap entries, Core Web Vitals, structured data markup — are out of scope and risk conflicting advice if handled here. The boundary is clear: `blogging-content-strategy-stinger` produces the content architecture and post briefs; `seo-aeo-wasp-drone` implements the technical substrate.

**Specific handoff triggers:**
- Schema markup (FAQPage, Article, HowTo) → `seo-aeo-wasp-drone`
- Sitemap inclusion or priority settings → `seo-aeo-wasp-drone`
- Core Web Vitals fixes → `seo-aeo-wasp-drone`
- robots.txt directives → `seo-aeo-wasp-drone`
- hreflang for multi-language → `seo-aeo-wasp-drone`
