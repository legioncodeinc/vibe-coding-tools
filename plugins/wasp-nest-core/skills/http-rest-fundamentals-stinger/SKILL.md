...
description: "Audit HTTP and REST behavior. Use for methods, status codes, headers, caching, CORS, conditional requests, or API semantics. Read README.md for the guide map."
---

# http-rest-fundamentals Stinger

Procedural arsenal for `http-rest-fundamentals-wasp-drone`, the HTTP and REST protocol authority in the Wasp Nest.
...
**Paired Drone:** `../../agents/http-rest-fundamentals-wasp-drone.md`
**Command Brief:** `ai-tools/command-briefs/http-rest-fundamentals-wasp-drone-command-brief.md`
...
Then pick the appropriate template from `templates/` for the deliverable the Drone is producing.
...
- **Do not audit authentication tokens, JWTs, or session cookies** -- hand off to `auth-wasp-drone`. Why: boundary prevents duplicate findings.
- **Do not audit TLS, cipher suites, or certificate validity** -- hand off to `devops-wasp-drone`. Why: same boundary rationale; this stinger stays at the application layer.
...
*Forged by `stinger-forge` from `http-rest-fundamentals-wasp-drone-command-brief.md` and `research/`. Part of The Wasp Nest by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
