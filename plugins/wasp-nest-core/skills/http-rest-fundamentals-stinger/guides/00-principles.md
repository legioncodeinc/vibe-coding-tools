...
| Concern | Owner |
|---|---|
| TLS, cipher suites, certificate validity | `devops-wasp-drone` |
| Authorization header semantics, JWT validation, OAuth flows | `auth-wasp-drone` |
| 401 vs 403 distinction (HTTP-layer) | `http-rest-fundamentals-wasp-drone` |
| WWW-Authenticate header format | `http-rest-fundamentals-wasp-drone` |
| OWASP-level security header enforcement (CSP, X-Frame-Options) | `security-wasp-drone` (flag here; hand off) |
| HTTP headers used as crawler hints (X-Robots-Tag, canonical) | `seo-aeo-wasp-drone` |
