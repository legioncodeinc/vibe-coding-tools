...
description: "Design CRM sync with HubSpot, Salesforce, Pipedrive, Attio, and peers. Use for field mapping, deduplication, or sync conflicts. Read README.md for the guide map."
---

# crm-integration-stinger

CRM connectivity playbook for `crm-integration-wasp-drone`. Synthesized from 10 source notes covering Merge.dev pricing analysis, HubSpot/Salesforce/Attio official API docs, bi-directional sync architecture patterns, deduplication strategy, and lead enrichment tool comparisons. Research window: November 2025 -- May 2026.

See `research/research-summary.md` for key findings and open questions. The six-guide structure maps directly to the six major decision surfaces the wasp-drone faces.
...
Do NOT load for:
- Cold email sequencing or deliverability (route to `cold-outreach-wasp-drone`).
- The product's own internal Person/Account schema (route to `db-wasp-drone`).
- Implementation of the sync backend in Django or Node.js (route to `python-wasp-drone`).
- Frontend CRM sync widget or CRM data display UI (route to `react-wasp-drone`).
- GDPR data residency for CRM data (flag and route to `security-wasp-drone`).
...
*Forged by `stinger-forge` from `crm-integration-wasp-drone-command-brief.md` and `research/`. Part of the Legion AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
