...
Every specialist Drone is paired with exactly one Stinger, and every specialist Stinger with exactly one Drone in the same installable unit. The Drone is persona plus guardrails; the Stinger is the knowledge it wields. A Drone dispatched without its Stinger loaded is a failed dispatch: terminate and re-dispatch. Direct-use skills are permitted only with explicit `metadata.hive-tier: standalone`; orchestrator skills use `metadata.hive-tier: orchestrator`.

The pairing is enforced by two verbatim text blocks that queen-wasp-stinger bakes into every component it forges:

- The **agent Critical Directive** at the top of every Drone body: load your core skill now, before any planning or execution, read all of it, supplement from the internet and knowledge base if it falls short, and here are the related skills.
...
Three core skills operate at the harness orchestrator level, wielded by the main agent rather than a subagent:

| Skill | Function |
|---|---|
| `pest-controller-suit` | Routing. The roster of every registered Drone, its domain, triggers, and guide. The orchestrator consults it to decide who owns a task |
| `queen-wasp-stinger` | Creation. The forge that builds and validates new rules, plugins, commands, Drones, and Stingers |
...
Other unpaired skills must explicitly declare `standalone` or `orchestrator` in frontmatter. A standalone skill does not claim a specialist Drone exists. The validator rejects an undeclared missing pair and also rejects a Drone paired to an exception skill.

## The closed loop

Every unit of Wasp Nest work follows the same circuit:

```mermaid
flowchart TD
    U[User invokes a Pest Controller Tool] --> B[Command loads pest-controller-suit]
    B --> R{Roster match?}
    R -- yes --> D[Dispatch Drone with arming line]
    R -- no --> F[queen-wasp-stinger forges the missing pair]
    F --> REG[Register in pest-controller-suit]
    REG --> D
    D --> S[Drone loads its Stinger per Critical Directive]
    S --> W[Drone executes inside its scope]
...
1. **Commands route through the pest-controller.** Every Pest Controller Tool loads `pest-controller-suit` first, so no work bypasses the roster. If no Drone owns the task, the orchestrator handles it inline or forges a new pair; it never improvises a Drone.
2. **Drones stay in their lanes.** Each Drone owns one domain and the files in its scope. During parallel sessions, no Drone modifies another Drone's active work. Missing inputs get batched clarifying questions, not placeholder guesses.
...
The Ship Gate text is a verbatim block that ends every development-focused command, Drone, and Stinger. Research-only components may omit it; anything that can touch code may not.
...
| Drones | `.claude/agents/` | `.claude/agents/`, also reads `.claude/agents/` | `agents.<role>` in config.toml | plugin `agents/` |
