...
Decision-grade reference for where the five Wasp Nest component types (Rules, Plugins, Commands, Agents, Skills) stand across the four harnesses (Claude Code, Cursor, ChatGPT Codex, Claude Cowork). Every claim traces to `references/research/distilled-research-articles.md`, which itself cites `raw/` sources. Nothing here comes from training data. Where the distilled doc flags a gap or a source conflict, that gap is stated here, not smoothed over.
...
## Wasp Nest term mapping

Wasp Nest naming is not harness naming. Use this table to translate before reading further.

| Wasp Nest term | Generic component type | Covered in section |
|---|---|---|
| Pest Controller and Tools | Commands | Commands |
| Drones | Agents (subagents) | Agents |
| Stingers | Skills | Skills |
| Repo-wide conventions | Rules | Rules |
| Bundled Drone + Stinger distribution | Plugins | Plugins |
| The Wasp Nest (whole framework) | (none) | Orchestrator level; see `pest-controller-suit`, `queen-wasp-stinger`, and `get-started-stinger`, with no single generic-component equivalent |
...
| Agents | Cursor | Yes | `.cursor/agents/<name>.md`, plus `.claude/agents/` and `.codex/agents/` fallback reads | native |
...
**Portability path.** `AGENTS.md` is the widest-reach baseline: read natively by Codex, read as one of Cursor's four rule types, and importable into Claude Code via `@AGENTS.md`. It reaches three of four harnesses with one file. Cowork has no rules-file surface at all. A Wasp Nest rules template can document the AGENTS.md/CLAUDE.md pairing, but Cowork rule content has to live in Global/Folder instructions set through the app, which is out of scope for a versioned file.
...
| Cursor | `.cursor/agents/` | Project |
| Cursor | `~/.cursor/agents/` | User, all projects |
...
**Portability path.** No single agent-definition file loads unmodified in all four harnesses. Claude Code and Cowork share a plugin-agent subset, while Cursor accepts only `name`, `description`, `model`, `readonly`, and `is_background` in its agent frontmatter. The Wasp Nest keeps Claude source fields such as `tools` and `isolation` and projects a Cursor-specific file under `.cursor/agents/` or the plugin's `cursor-agents/` directory. Codex receives TOML agent roles from the build. [raw/claude-code--agents--sub-agents-official-docs.md, raw/cursor--agents--subagents-docs.md, raw/codex--rules--config-reference.md]
