---
name: "{stinger-name}"
description: "{What this skill does and when to use it, in 200 characters or fewer. Put decision trees in README.md or guides/ and link them from the body.}"
...
metadata:
  hive-drone: "{paired-drone-name, or 'none' for orchestrator-level stingers}"
  domain: "{one or two words: what part of the Wasp Nest this covers}"
  pair-drone: "{same as hive-drone, kept for readability if your roster convention uses this key name instead}"
---
...
- `README.md`, {load when: the task needs the entry-point decision tree or guide map; omit this line when no README exists}
...
- [{related-drone-name}](../../agents/{related-drone-file}) - {Description of the paired agent and when the orchestrator should delegate to it instead of running this stinger inline.}
