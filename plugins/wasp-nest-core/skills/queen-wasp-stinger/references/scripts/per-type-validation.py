#!/usr/bin/env python3
"""
per-type-validation.py -- Project Wasp Nest (queen-wasp-stinger)
...
def is_placeholder(value):
    """Unfilled Wasp Nest template values look like {this}. Report them as INFO
    ...
    if "cowork" in harnesses:
        ...
        if skills_dir.is_dir():
            for skill_folder in sorted(p for p in skills_dir.iterdir() if p.is_dir()):
                if not (skill_folder / "SKILL.md").is_file():
                    report.add("ERROR", rel_label(root, skill_folder / "SKILL.md"), "skill directory has no SKILL.md")
                else:
