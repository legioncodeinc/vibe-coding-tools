...
## 5. The deploy step: handed to `db-wasp-drone` for migrations, cloud-platform Drone for runtime

The image is pushed; what runs it is downstream. This Drone typically wires:
...
deploy:
  ...
  steps:
    ...
    - name: Run DB migrations
      ...
      # ↑ owned by db-wasp-drone; devops-wasp-drone wires it in
    - name: Deploy to Fargate / ECS
      run: aws ecs update-service ...
```

The migration step is `db-wasp-drone`'s domain: its content (SQL, ORM commands, runtime checks) is theirs. `devops-wasp-drone` ensures:
