...
- [ ] `security-wasp-drone`: {{if any secret-leak / CVE / token-scope concern surfaced}}
- [ ] `db-wasp-drone`: {{if migration step needs authoring or schema-level concern flagged}}
- [ ] `library-wasp-drone`: {{if pipeline change is large enough to warrant PRD authoring}}
- [ ] `quality-wasp-drone`: {{post-implementation verification}}
- [ ] `react-wasp-drone`: {{if Node version / workspace decisions affect the React app}}
