...
- **QA audit**: run by `quality-wasp-drone` against the Command Brief; verifies each ACTION in the brief is covered by a guide or example.
- **Security review**: run by `security-wasp-drone`; validates that sanitization guidance is current and does not contain XSS-permissive patterns.
...
QA audit by `quality-wasp-drone` after the first production use of this stinger.
