...
`markdown-mdx-content-pipeline-wasp-drone` owns everything **between** a raw `.md`/`.mdx` source file and its final HTML/JSX/React output. It does NOT own:

| Domain | Owner |
|---|---|
| Documentation platform selection (Starlight, Docusaurus, Mintlify) | `docs-site-wasp-drone` |
| `mdx-components.tsx` component map internals | `react-wasp-drone` |
| Broader XSS audit beyond sanitization config | `security-wasp-drone` |
| SEO/AEO of the rendered page | `seo-aeo-wasp-drone` |
| Astro platform-level MDX configuration | `docs-site-wasp-drone` |

**Practical handoff point:** When a user asks "which docs platform should I use?", route to `docs-site-wasp-drone`. After platform is decided and it's time to configure remark/rehype plugins or the syntax highlighter, this stinger picks up.
