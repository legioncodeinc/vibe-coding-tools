...
| Question type | Owner |
|---|---|
| Vercel adapter config, runtime choice, ISR/caching, images, firewall, cost control, Neon integration choice | `vercel-wasp-drone` (this skill owns the Actions side and the two systems' interaction, not Vercel's own config) |
| Drizzle schema design, migration command semantics, RLS, connection pooling | `neon-drizzle-wasp-drone` (this skill owns CI gating logic only) |
| Doppler project/config model, rotation, audit logs, Vercel sync | `doppler-wasp-drone` (this skill owns secret delivery into Actions workflows only) |
| Branch protection / ruleset settings configuration, repo-hygiene scoring | `github-repo-health-wasp-drone` (this skill owns making a required check satisfiable, not configuring the ruleset) |
| Svelte 5 component/runes correctness | `svelte-wasp-drone` |
| CVE deep audit, secret-leak forensics, supply-chain correctness | `security-wasp-drone` (you surface concerns) |
| Dependency / lockfile CVE triage | `dependency-audit-wasp-drone` (you wire the step) |
| Runtime TS/Node source design, ESM/module-resolution (npm-package case) | `typescript-node-wasp-drone` |
| Post-implementation verification | `quality-wasp-drone` |

You surface concerns but don't author the security audit yourself.

**Close-out chain on any pipeline change:** hand to `security-wasp-drone` first (publish-surface / secret check), then `quality-wasp-drone` (gate parity verification), then `github-repo-health-wasp-drone` for the Ship Gate's final orchestrator-level step - see `SKILL.md`'s Ship Gate section.

## Scope explicitly excluded (both cases)

- **Runtime business logic.** This Drone stops at "the pipeline builds, gates green, and ships/deploys." What the code does at runtime is `svelte-wasp-drone` (app case) or `typescript-node-wasp-drone` (npm-package case).
- **Vercel platform configuration itself** (adapter options, ISR, images, firewall) - that's `vercel-wasp-drone`'s territory; this skill owns the Actions side and the interaction between the two.
