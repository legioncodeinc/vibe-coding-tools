...
description: "Design CI and release gates for SvelteKit on Vercel or npm packages. Use for Actions jobs, migrations, caching, or publishing. Read guides/ for the decision path."
license: AGPL-3.0-or-later
compatibility: Claude Code, Cursor, ChatGPT Codex, Claude Cowork.
metadata:
  hive-tier: stinger
  hive-drone: ci-release-wasp-drone
...
Use when the user says "design our CI", "audit our workflows", "add a CI job", "why did Vercel build twice", "gate this migration in CI", "wire Doppler into Actions", "our Playwright job is slow", "our required check never passes", "do we need semantic-release" - or, for the legacy case, "review our build", "the bundle is wrong", "the version is out of sync", "we leaked a secret on publish", "tree-sitter broke on install" - or when `ci-release-wasp-drone` is invoked.

Do NOT use for Vercel's own platform configuration (adapter options, ISR/caching, images, firewall, cost control - `vercel-wasp-drone`), Drizzle schema/migration mechanics themselves (`neon-drizzle-wasp-drone`), Doppler's own project/config/rotation model (`doppler-wasp-drone`), branch protection/ruleset settings configuration (`github-repo-health-wasp-drone`), Svelte 5 component correctness (`svelte-wasp-drone`), Docker/Compose/Depot pipelines (`devops-wasp-drone`), security CVE deep audits (`security-wasp-drone` - this skill surfaces concerns and hands off), changelog/release-notes prose (`changelog-release-notes-wasp-drone`), or dependency CVE triage (`dependency-audit-wasp-drone`).

You are equipping **ci-release-wasp-drone** - the Wasp Nest's authority on how this repo's app builds, tests, gates, and ships, and (in the legacy case) how a published package builds, gates, and ships to npm.
...
3. **Read `guides/00-principles.md` before writing any finding.** The severity rubric and cross-Drone handoff rules live there.
...
- **Vercel platform configuration itself** (adapter, runtime, ISR/caching, images, firewall, cost, Neon integration choice) → `vercel-wasp-drone`. This Drone owns the GitHub Actions side and the two systems' interaction, not Vercel's own config.
- **Drizzle schema design, migration command semantics, RLS, connection pooling** → `neon-drizzle-wasp-drone`. This Drone owns CI gating logic around migrations, not migration mechanics themselves.
- **Doppler project/config model, rotation, audit logs, Vercel sync** → `doppler-wasp-drone`. This Drone owns secret delivery into Actions workflows, not the platform itself.
- **Branch protection / ruleset settings configuration, repo-hygiene scoring** → `github-repo-health-wasp-drone`. This Drone owns making a required check satisfiable in CI; that Drone owns configuring the ruleset. Also part of the Ship Gate below.
- **Svelte 5 component/runes correctness** → `svelte-wasp-drone`.
- **Docker/Compose/Depot pipelines** (not applicable to this repo's Vercel-deployed stack, but relevant if a sibling service containerizes) → `devops-wasp-drone`.
- **CVE deep audit of dependencies / secret-leak forensics / supply-chain correctness** → `security-wasp-drone`. This Drone *surfaces* concerns; the audit is `security-wasp-drone`'s job.
- **Dependency version / CVE triage of the lockfile** → `dependency-audit-wasp-drone`. This Drone wires the audit step; that Drone owns the verdict.
- **Release-notes / changelog prose + announcement** (legacy case) → `changelog-release-notes-wasp-drone`. This Drone owns the *mechanics* of cutting a release; the announcement copy is theirs.
- **Runtime TS/Node source design, ESM/module resolution decisions** (legacy case) → `typescript-node-wasp-drone`.
- **Harness integration semantics** (legacy case, what a harness bundle must export) → `harness-integration-wasp-drone`.
- **Post-implementation verification** → `quality-wasp-drone`.
- **Close-out chain on any pipeline change:** hand to `security-wasp-drone` first (publish-surface / secret check), then `quality-wasp-drone` (gate parity verification), then the Ship Gate below.
...
- `guides/00-principles.md` - classification step (app vs package), first-move checklist, severity rubric, cross-Drone boundaries. Read first on every invocation, regardless of case.
