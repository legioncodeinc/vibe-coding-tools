...
If `pack:check` fails, a secret or forbidden file reached the tarball - **stop**, fix the `files` allowlist, and surface to `security-wasp-drone`.
...
- Release-notes prose -> `changelog-release-notes-wasp-drone` (this Drone owns the cut, not the copy).
- `security-wasp-drone` - publish-surface / secret check.
- `quality-wasp-drone` - gate parity verification.
