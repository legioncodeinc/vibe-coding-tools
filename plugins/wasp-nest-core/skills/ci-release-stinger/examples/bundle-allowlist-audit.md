...
- Fix: remove it, or add a pack-check rule. If it ever carried a real value, surface to `security-wasp-drone`.
...
Produce the audit with `templates/bundle-audit.md`: one Must-fix (hermes not shipped), any allowlist hygiene findings, version-sync status, and the close-out handoff to `security-wasp-drone` if a secret-bearing path appeared.
