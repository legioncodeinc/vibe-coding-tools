...
**Treat the credential as compromised immediately.** Escalate to `security-wasp-drone` for credential rotation in parallel with history cleanup.
...
## Step 6: Credential rotation (escalate to security-wasp-drone)
...
This step is `security-wasp-drone`'s domain - escalate immediately.
