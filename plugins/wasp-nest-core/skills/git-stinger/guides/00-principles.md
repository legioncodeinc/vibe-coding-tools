...
The five non-negotiable rules that govern every `git-wasp-drone` response.
...
| Trigger | Escalate to |
|---|---|
| Credential rotation after secrets removal | `security-wasp-drone` |
| Server-side hooks (`pre-receive`, `update`, `post-receive`) | `ci-release-wasp-drone` |
| CI/CD pipeline using Git events | `ci-release-wasp-drone` |
| GitHub/GitLab REST API calls | inline or `ci-release-wasp-drone` |
