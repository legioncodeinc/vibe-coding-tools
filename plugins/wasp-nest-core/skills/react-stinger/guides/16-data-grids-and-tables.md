...
- **Token / spacing / typography of the table surface** → `ux-ui-svelte-wasp-drone`.
- **Server-side row model API design (cursor pagination, filter pushdown)** → `db-wasp-drone`.
- **License procurement (AG Grid, Handsontable, MUI X)** → `library-wasp-drone` PRD.
