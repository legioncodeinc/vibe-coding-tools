...
- Themed via your CSS variables (the same OKLCH tokens `ux-ui-svelte-wasp-drone` defines).
...
Note the CSS variable references: that's how you keep the chart in lockstep with `ux-ui-svelte-wasp-drone`'s tokens.
...
> **[Must-fix]** `src/features/dashboard/Pie.tsx:12`: colors are hex literals (`#ff6b6b`). Replace with `var(--chart-1)` etc. to inherit dark/light mode and brand changes. Hand off to `ux-ui-svelte-wasp-drone` for token mapping.
...
- **Color, gradient, and contrast token decisions** → `ux-ui-svelte-wasp-drone`. Every chart color is a token, never a literal.
- **Data layer (cursor pagination, downsampling, server-side aggregations)** → `db-wasp-drone` for the API; this guide assumes data already lives in the component.
