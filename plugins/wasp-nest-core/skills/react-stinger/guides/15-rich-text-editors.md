...
- **Realtime collaboration infra (Yjs server, Liveblocks, PartyKit)** → flag for `devops-wasp-drone` or library-wasp-drone PRD.
- **Token / typography / spacing of the editor surface** → `ux-ui-svelte-wasp-drone`.
- **AI completions inside the editor** → `mind-wasp-drone`.
- **Server-side sanitization of editor HTML/JSON** → `security-wasp-drone`.
