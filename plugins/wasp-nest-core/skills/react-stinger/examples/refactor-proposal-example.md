...
A phased refactor plan. Output shape when the invocation is "propose a refactor". The final artifact is handed to `library-wasp-drone` for PRD authoring.
...
5. Replace auth-scroll-your-own → Auth.js or Clerk (involves security-wasp-drone; out of this ADR).
...
4. Add Server Actions for mutations (guarded with Zod + auth; hand-off to `security-wasp-drone` per action).
...
- **This plan → `library-wasp-drone`** for PRD authoring. The PRD will expand Phase 3 and Phase 6 into user-visible milestones, acceptance criteria, risk register, rollback plan.
- **Per-phase ADRs → repo `library/knowledge/private/architecture/ADR-<n>-<topic>.md`.**
- **Post-phase QA → `quality-wasp-drone`.**
