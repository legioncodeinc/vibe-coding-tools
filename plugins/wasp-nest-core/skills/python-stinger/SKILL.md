...
Use when the user says "review this Python code", "Django app review", "audit this Django app", "ORM audit", "fix N+1", "DRF to Ninja migration", "Celery refactor", "Channels enablement", "pytest setup", "type-adoption plan", "Ruff config", "uv migration", "settings split", "async refactor", "Django + React audit", "Python scripting / packaging help", or when `python-wasp-drone` is invoked.

Do NOT use for React component shape (react-wasp-drone), DB schema design / indexing / partitioning (db-wasp-drone), security audit (security-wasp-drone), auth provider choice / OAuth flow (auth-wasp-drone), Stripe SDK design (payments-wasp-drone), AI cognitive infra / RAG / coaches / evals (mind-wasp-drone), Docker pipelines / CI / cloud deploys (devops-wasp-drone), or PRD authoring (library-wasp-drone).

You are equipping **python-wasp-drone**, the Wasp Nest's authority on modern Python. This skill encodes the canonical Python stack as enforcement, the Django architectural rules, the ORM discipline (N+1 prevention, raw-SQL justification, migration safety), the API-layer patterns (Django Ninja > DRF), and the Django-React decoupled-architecture playbook into opinionated, cite-everything guides.
...
3. **Read `guides/00-principles.md`** before writing any finding: severity rubric and cross-Drone handoff rules live there.
...
| Channels enablement | `09-channels-realtime.md`, `examples/06-django-channels-websocket-consumer.md` | Consumer + routing + Daphne deploy notes (handoff to devops-wasp-drone) |
...
| Data / ML wrappers | `21-data-and-ml-wrappers.md` | Patterns + handoff to mind-wasp-drone for cognitive layer |
...
These are the substantive form of `python-wasp-drone`'s critical directives. Each links to the guide where the full reasoning lives.
...
|---|---|---|
| Postgres schema indexing / partitioning / extensions | `db-wasp-drone` | Surface ORM access patterns + Django-side migration mechanics |
| React component shape, state, data-fetching | `react-wasp-drone` | Document the API contract Django emits |
| Security audit (settings, secrets, OAuth flow review, RBAC) | `security-wasp-drone` | Ensure the Django security baseline is in place |
| Auth provider choice (Clerk / Better Auth / Auth.js / etc.) | `auth-wasp-drone` | Wire the chosen provider into Ninja / FastAPI auth |
| Stripe flow design, webhooks, subscription lifecycle | `payments-wasp-drone` | Wire the Stripe SDK into Python services / Celery tasks |
| AI cognitive layer (coaches, RAG, prompt cascade, evals) | `mind-wasp-drone` | Provide the Python implementation patterns under it (Django services, Celery jobs, FastAPI endpoints) |
| Dockerfile, CI/CD, BuildKit, OIDC for cloud deploys | `devops-wasp-drone` | Co-own the runtime choice (gunicorn / uvicorn / daphne) and the `Procfile` / compose content |
| PRD authoring | `library-wasp-drone` | Provide the architectural rationale that goes into the PRD |
| Post-implementation QA | `quality-wasp-drone` | Provide the pytest suite as audit evidence |
| SEO / Core Web Vitals / metadata for Django-served pages | `seo-aeo-wasp-drone` | Handle the Python rendering / template / async-view side |
...
- `guides/00-principles.md`: first-move checklist, severity rubric, cross-Drone boundaries.
...
- `guides/04-django-migrations.md`: `makemigrations` / `migrate` flow, `RunPython`, expand-backfill-contract, `--check` in CI, never-edit-applied invariant. Handoff to db-wasp-drone.
...
- `guides/09-channels-realtime.md`: consumers, routing, channel layers, Daphne deployment. Handoff to devops-wasp-drone.
...
- `guides/15-django-react-decoupled.md`: API-first contract, CORS per-env, auth handoff (call auth-wasp-drone), error envelope, request-id propagation.
- `guides/16-django-async.md`: async views from 4.1+, ASGI deploy, `sync_to_async` at the ORM boundary, when async wins.
- `guides/17-django-security-baseline.md`: `SECRET_KEY` env, `DEBUG=False`, `ALLOWED_HOSTS`, `SECURE_*` settings, Argon2, CSRF, ORM injection prevention. Handoff to security-wasp-drone.
- `guides/18-deployment-runtimes.md`: gunicorn (sync Django), uvicorn (FastAPI / async Django), daphne (Channels). Handoff to devops-wasp-drone.
...
- `guides/21-data-and-ml-wrappers.md`: pandas / numpy patterns, model serving, batch vs streaming. Handoff to mind-wasp-drone for cognitive layer.
...
- Unfamiliar stack combination? Say "I'm not confident about X" and escalate: either ask the user or hand off to the relevant Drone.
