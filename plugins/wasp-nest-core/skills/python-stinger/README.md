...
Cursor skill that equips **python-wasp-drone** to be the authority on modern Python: opinionated, modern, grounded in production-proven patterns rather than tutorial tropes. Encodes a canonical stack as enforcement, applies it across Django app architecture, the API layer, async work, realtime, testing, type discipline, packaging, and the Django-React decoupled-architecture surface.
...
- **Does not own:** React component shape (`react-wasp-drone`), DB schema design (`db-wasp-drone`), security audit (`security-wasp-drone`), auth provider choice (`auth-wasp-drone`), Stripe flow design (`payments-wasp-drone`), AI cognitive infra (`mind-wasp-drone`), Docker / CI / cloud deploys (`devops-wasp-drone`), PRD authoring (`library-wasp-drone`), post-implementation QA (`quality-wasp-drone`).
...
| React component shape, state management, data fetching | `react-wasp-drone` |
| Security audit (settings, secrets, OAuth flow review, RBAC) | `security-wasp-drone` |
| Auth provider choice (Clerk / Better Auth / Auth.js / etc.) | `auth-wasp-drone` |
| Stripe flow design, webhooks, subscription lifecycle | `payments-wasp-drone` |
| AI cognitive layer (coaches, RAG, prompt cascade, evals) | `mind-wasp-drone` |
| Dockerfile, CI/CD, BuildKit, OIDC for cloud deploys | `devops-wasp-drone` |
| PRD authoring | `library-wasp-drone` |
| Post-implementation QA | `quality-wasp-drone` |
| SEO / Core Web Vitals for Django-served pages | `seo-aeo-wasp-drone` |
