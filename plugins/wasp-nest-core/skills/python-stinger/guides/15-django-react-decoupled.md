...
5. **Auth provider choice is `auth-wasp-drone`'s call.** This guide covers the Python wiring of session / JWT / external-token patterns.
...
When the app spans domains, includes mobile clients, or third-party API consumers. Hand the **provider choice** to `auth-wasp-drone`. The Python wiring:
...
Refresh-token flow, MFA, RBAC, OAuth callbacks: all `auth-wasp-drone` territory.

## Auth pattern: external provider (Clerk / WorkOS / Auth0 / Supabase Auth)

Provider SDK on the React side issues a token; Django Ninja validates via a custom `HttpBearer` that calls the provider's verify endpoint or validates the JWT against the provider's JWKS. Provider choice is `auth-wasp-drone`; the wiring is the same shape as the JWT pattern above.
...
Discuss the React tooling choice with `react-wasp-drone`.
...
- **Auth provider choice** → `auth-wasp-drone`.
- **React component / state / data-fetching shape** → `react-wasp-drone`.
- **Security audit of the auth surface** → `security-wasp-drone`.
