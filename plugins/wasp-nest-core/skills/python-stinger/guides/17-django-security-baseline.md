...
The non-negotiable settings for production Django. The full security audit hands off to `security-wasp-drone`; this guide ensures the floor is in place.
...
- **Production secrets via the deploy platform's secret manager**: environment variables injected at boot. Hand off to `devops-wasp-drone` for the deploy-side wiring.
...
## Handoff to security-wasp-drone

- OAuth flow review, RBAC correctness, secret-rotation policy, threat modeling, dependency-vulnerability scanning, penetration test prep: all `security-wasp-drone`.
- Ensuring the baseline above is in place + `check --deploy` clean: this Drone.
