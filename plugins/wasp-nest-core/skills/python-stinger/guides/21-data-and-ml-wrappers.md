...
Patterns when Python is the analytical / ML runtime. The cognitive layer (coaches, RAG, evals, vector DBs) hands off to `mind-wasp-drone`; the Python implementation patterns underneath stay here.

## Where the boundary is

| Concern | Owner |
|---|---|
| Coach architecture, prompt cascade, RAG strategy, evals, vector DB choice, embedding model choice | **`mind-wasp-drone`** |
...
For GPU serving, batch requests, or heavy-traffic scenarios, consider Triton, vLLM, or Ray Serve: that's an ML-platform decision, surface it to `mind-wasp-drone`.
...
The Python wiring of an LLM call (the `mind-wasp-drone` defines the cognitive design):
...
- Behind a configurable gateway URL (the `mind-wasp-drone` picks the gateway).
- Timed out: LLM calls can hang for a minute or more.
- Logged with token counts for cost / observability (handoff to `mind-wasp-drone` for the AI-trace shape).
