...
6. **The DB-side concern (locks, indexes, constraints, partitioning) hands off to `db-wasp-drone`.** This guide is about the Django mechanics.
...
## Handoff to db-wasp-drone

- Postgres-side concerns (lock modes, partitioning, index strategy, vacuum / analyze, replication considerations) → `db-wasp-drone`.
