...
Python service emits JSON; React consumes it. CORS configured per-environment. Auth is a deliberate decision (session vs JWT vs external, call `auth-wasp-drone`). Django templates are out of scope unless the project is admin-only or a server-rendered legacy. Source: `guides/15-django-react-decoupled.md`.
...
- [ ] Cross-Drone handoff lines clear: escalate at the boundary, don't author work the other Drone owns.
...
| Question | Owner |
|---|---|
| Postgres schema indexing / partitioning / extensions | `db-wasp-drone` |
| React component shape, state, data fetching | `react-wasp-drone` |
| Security audit (settings, secrets, OAuth flow, RBAC) | `security-wasp-drone` |
| Auth provider choice (Clerk / Better Auth / etc.) | `auth-wasp-drone` |
| Stripe flow design, webhooks, subscriptions | `payments-wasp-drone` |
| AI cognitive layer (coaches, RAG, evals, vector DB) | `mind-wasp-drone` |
| Dockerfile, CI/CD, BuildKit, cloud deploys | `devops-wasp-drone` |
| PRD authoring | `library-wasp-drone` |
| Post-refactor QA verification | `quality-wasp-drone` |
| SEO / metadata / Core Web Vitals | `seo-aeo-wasp-drone` |
...
- **AI cognitive layer.** Python is the runtime, but coaches / RAG / prompt cascade / evals belong to `mind-wasp-drone`. Surface the Python implementation patterns; don't author the cognitive design.
- **DB schema engineering.** Indexing strategies, partitioning, vendor extensions, raw schema design → `db-wasp-drone`. The Django ORM access patterns, queryset optimization, Django-side migration mechanics → this Stinger.
- **OAuth provider integration.** The Python wiring (Ninja `HttpBearer` subclass, FastAPI dependency, Django session config) is here; the provider choice is `auth-wasp-drone`.
