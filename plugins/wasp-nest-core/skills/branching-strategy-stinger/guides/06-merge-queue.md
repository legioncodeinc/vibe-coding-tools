...
- If the Merge Queue setup requires changes to GitHub Actions workflows: route to `ci-release-wasp-drone`.
- If the Merge Queue requires branch protection ruleset changes: route to `github-repo-health-wasp-drone`.
- `branching-strategy-wasp-drone` owns the decision of whether to use a merge queue; the other Drones own the configuratio