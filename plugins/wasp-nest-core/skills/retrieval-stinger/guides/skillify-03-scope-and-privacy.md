...
Scope is a privacy boundary, but it is not a hardened security control on its own - the PII and access-control audit (is sensitive content ending up in a mined skill at all, even a `me` one) is security-wasp-drone's. retrieval-wasp-drone flags the scope-respect bug with file:line; the deeper audit is theirs.
...
5. **Hand PII / sensitive-content questions to security-wasp-drone** - "should this content be in a skill at all" is their call.
