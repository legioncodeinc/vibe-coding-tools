...
How recall consumes vectors. This guide covers the column/dimension contract, the toggles, and the null-vector handshake. The daemon that *produces* vectors (lifecycle, model, quantization) belongs to embeddings-runtime-wasp-drone - this guide stops at the boundary.
...
The dimension is set by the model: `nomic-ai/nomic-embed-text-v1.5` (q8) outputs 768. Recall does not choose the dimension; it inherits it. Changing it is a schema event (vector-store-wasp-drone), not a recall tuning knob.
...
| Recall (this Drone) | Daemon (embeddings-runtime) |
|---|---|
...
When a recall finding traces back to "the daemon was down" or "warmup was slow", state the symptom and hand the daemon mechanics to embeddings-runtime-wasp-drone.
