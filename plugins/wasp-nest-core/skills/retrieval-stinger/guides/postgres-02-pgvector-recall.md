...
| Question | Owner |
|---|---|
| What dimension should the `vector` column be? | vector-store-wasp-drone |
| HNSW or IVFFlat, and what parameters? | vector-store-wasp-drone |
| Which operator class does the index need? | vector-store-wasp-drone |
| Which distance operator should *this query* use, and does it match the index? | retrieval-wasp-drone (this guide) |
| Should this query run vector-only, lexical-only, or hybrid? | retrieval-wasp-drone (`postgres-03-rrf-fusion.md`) |
| Is recall good enough at the chosen k? | retrieval-wasp-drone (`postgres-06-recall-quality-eval.md`) |

If a finding is about the column definition or the index build, hand it to vector-store-wasp-drone. If it's about whether a *query* is using the right operator, weighting the vector arm correctly, or getting good recall, it's this Stinger's.
...
5. **Column/index questions** (dimension, HNSW vs IVFFlat, build parameters) get handed to vector-store-wasp-drone, not answered here.
