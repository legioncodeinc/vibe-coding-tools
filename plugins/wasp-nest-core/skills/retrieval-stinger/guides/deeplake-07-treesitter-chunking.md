...
The graph's *recall* (querying the `codebase` table for structural matches) is this Drone's. The Deep Lake `codebase` table *schema* is vector-store-wasp-drone's. A column or DDL change to `codebase` is handed to them.
