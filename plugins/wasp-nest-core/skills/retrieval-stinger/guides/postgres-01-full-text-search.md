...
Table/column shape and GIN index placement are collaborative with `vector-store-wasp-drone` when the same table also carries a `vector` column (the common case for a hybrid-search table, see `postgres-03-rrf-fusion.md`); this guide owns the lexical query/ranking correctness, `vector-store-stinger` owns the storage/index side.
