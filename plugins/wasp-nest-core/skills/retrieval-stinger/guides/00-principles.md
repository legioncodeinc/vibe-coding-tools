...
- **Vector column shape, dimension, index type (HNSW/IVFFlat), operator class, schema/DDL, Deep Lake table schema** -> **`vector-store-wasp-drone`**. This Stinger owns the recall *query* (which operator, which weighting, is recall good enough); vector-store-wasp-drone owns the column and index it queries against.
- **The embedding model, daemon, quantization, warmup, batching** -> **`embeddings-runtime-wasp-drone`**. This Stinger owns how recall *consumes* vectors and the null/missing-embedding fallback contract; the model and pipeline that *produce* them are theirs.
