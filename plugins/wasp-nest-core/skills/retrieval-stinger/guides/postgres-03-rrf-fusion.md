...
The column shape and index choice backing each arm belong to `vector-store-wasp-drone`; this guide owns the fusion query and its tuning. The embedding model producing `query_embedding` belongs to `embeddings-runtime-wasp-drone`.
