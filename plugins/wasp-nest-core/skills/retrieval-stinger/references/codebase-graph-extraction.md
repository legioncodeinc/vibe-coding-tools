...
Querying the `codebase` table for structural matches is retrieval-wasp-drone's. The table schema/DDL is vector-store-wasp-drone's. Language coverage is bounded by available tree-sitter grammars; an unsupported language yields file-level nodes only, and that gap should be stated.
