...
This is the semantic half of hybrid recall. It is blended with the lexical (BM25/ILIKE) arm via `deeplake_hybrid_record` (`hybrid-weighting.md`). The dimension and column DDL are owned by vector-store-wasp-drone; the daemon producing the vectors by embeddings-runtime-wasp-drone. retrieval-wasp-drone owns the query.
