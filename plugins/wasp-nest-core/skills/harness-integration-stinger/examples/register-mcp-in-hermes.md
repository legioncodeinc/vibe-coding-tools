...
*Part of the Wasp Nestmind six-host case study - see `examples/case-study-hivemind-six-host-installer.md` for the full context. Hermes is not one of The Wasp Nest's four current harnesses; this is retained as a worked MCP-registration pattern reference. This worked example predates and is not superseded by the case study file.*

**Demonstrates:** `guides/03-mcp-registration.md`, `guides/05-portability-and-contracts.md`

This example registers the Wasp Nestmind MCP server (`src/mcp/server.ts`) under `mcp_servers.hivemind` in `~/.hermes/config.yaml`, idempotently.
