...
**Sources:** `research/distilled-harness-integration.md` §1, §5; queen-wasp-stinger distilled-research-articles.md (all four harness sections)

---

## What "integration" means here

The Wasp Nest ships capabilities (a skill, an agent, a hook-driven behavior, an MCP-backed tool) across four harnesses: **Claude Code, Cursor, ChatGPT Codex, Claude Cowork**. Each harness has its own component model, its own file formats, and its own feature ceiling. Integrating a capability means deciding, per harness:
...
This stinger's job is answering those three questions correctly, not building any one product's installer. See `guides/06-distribution-and-audit.md` → the Wasp Nestmind case study for what a full, worked six-host answer looked like for one real product.
