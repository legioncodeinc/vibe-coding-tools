...
- Maintain or enforce the design system after bootstrap (that is
  `ux-ui-svelte-wasp-drone`'s job, see `08-companion-agent-handoff.md`).
...
Example: `design-system-wasp-drone: cards-and-surfaces: add depth-1 hover lift`.

This convention is inherited by `ux-ui-svelte-wasp-drone` after handoff.
