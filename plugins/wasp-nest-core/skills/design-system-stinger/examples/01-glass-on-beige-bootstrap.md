...
A worked example of `design-system-wasp-drone` bootstrapping a new design
...
`README.md` names `ux-ui-svelte-wasp-drone` as the owner. Status table shows:
...
`design-system-wasp-drone` exits. `ux-ui-svelte-wasp-drone` picks up.
