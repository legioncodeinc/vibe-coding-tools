...
A worked example of `design-system-wasp-drone` bootstrapping a design system
for a product that already ships with messy, unsystematic CSS. The Drone
must **both** produce the canonical seven-artifact folder AND write the
"Replaces (in current code)" sections that tell `ux-ui-svelte-wasp-drone` exactly
...
the migration plan to `ux-ui-svelte-wasp-drone`).
...
PURGE (flag for ux-ui-svelte-wasp-drone to refactor):
...
2026-04-24. The following migrations are in scope for `ux-ui-svelte-wasp-drone`
...
(authored by `ux-ui-svelte-wasp-drone` post-handoff).
...
The table IS the migration plan. `ux-ui-svelte-wasp-drone` opens it, and
knows what to do.
...
`README.md` for PulseCheck adds a section `ux-ui-svelte-wasp-drone` wouldn't
otherwise need:
...
refactor is tracked separately by `ux-ui-svelte-wasp-drone` in the host repo's
`library/requirements/reports/ux-ui/<date>-pulsecheck-migration.md`. Expected duration:
3–4 weeks of incremental PRs.
```

`design-system-wasp-drone` is done. The system is designed; the code
still needs to be made to match, but that is `ux-ui-svelte-wasp-drone`'s job.
...
5. **The handoff to `ux-ui-svelte-wasp-drone` carries a migration backlog**, not
   just a system. The enforcing Drone starts with work queued up.
...
- `../guides/08-companion-agent-handoff.md`, how the migration backlog
  passes to `ux-ui-svelte-wasp-drone`.
