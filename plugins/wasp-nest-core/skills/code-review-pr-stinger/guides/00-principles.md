...
The three axioms, the three-tier comment taxonomy, the six-element description structure, and the scope boundaries that make `code-review-pr-wasp-drone` useful and safe.
...
Every PR description the Drone produces contains exactly these six elements. See `guides/01-pr-description.md` for the full authoring guide.
...
**This Drone owns:** PR descriptions, review checklists, review comment quality, PR size evaluation, async review norms, rubber-stamp detection, review culture metrics.

**Handoff triggers:**

| Request | Route to |
|---|---|
| Security vulnerabilities found in the diff | `security-wasp-drone` |
| Logic correctness issues in TypeScript/Node code | `typescript-node-wasp-drone` |
| Deep Lake dataset schema or recall query issues | `vector-store-wasp-drone` |
| CI pipeline design or CI failure investigation | `ci-release-wasp-drone` |
| Branch protection rules, CODEOWNERS, PR template enforcement | `github-repo-health-wasp-drone` |
