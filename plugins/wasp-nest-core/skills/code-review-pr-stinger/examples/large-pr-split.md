...
A worked example of `code-review-pr-wasp-drone` evaluating a large PR (643 changed lines, 4 mixed concerns) and proposing a concrete split into reviewable increments.
...
**Rationale:** Dataset-only change; can be reviewed by whoever owns the Deep Lake layer without retrieval context. Ships independently. Route any schema questions to `vector-store-wasp-drone`.
