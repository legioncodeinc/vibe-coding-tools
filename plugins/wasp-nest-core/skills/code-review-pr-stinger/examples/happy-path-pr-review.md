...
A worked end-to-end example of `code-review-pr-wasp-drone` handling a well-scoped PR: description audit, checklist generation, and comment coaching.
...
>
> If we want stricter validation, consider the `validator` library's `isEmail()` which handles the RFC edge cases. Trade-off: adds a dependency (and `dependency-audit-wasp-drone` review); benefit: maintained regex we don't have to own.
>
