...
**Hand schema + RLS migration to `db-wasp-drone`.** auth-wasp-drone flags the requirement; db-wasp-drone writes the SQL.
...
- The RLS policy set (handed to `db-wasp-drone` first; then audited by `security-wasp-drone`).
