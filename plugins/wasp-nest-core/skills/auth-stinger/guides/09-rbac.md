...
Hand RLS migration to `db-wasp-drone`. auth-wasp-drone flags the requirement; db-wasp-drone writes the SQL with proper indexing and the right `user_tenants` relationship.
...
- These are `db-wasp-drone` decisions; auth-wasp-drone flags the requirement.
...
- The RLS policies (handed to `db-wasp-drone` first).
