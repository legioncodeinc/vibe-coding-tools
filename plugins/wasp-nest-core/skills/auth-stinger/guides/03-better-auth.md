...
## Schema requirements (flag for db-wasp-drone)

Better Auth ships migrations for: `user`, `session`, `account`, `verification`. Plugins add: `organization`, `member`, `invitation`, `twoFactor`, `passkey`. Source the schema from the Better Auth CLI and hand it to `db-wasp-drone`:

```bash
npx @better-auth/cli@latest generate
```

`db-wasp-drone` owns: indexing strategy, RLS policies if multi-tenant, ON DELETE behavior on FKs, partition strategy at scale.
...
Decisions to flag for `security-wasp-drone`:
...
- The `users` schema decisions (handed to `db-wasp-drone` first).
