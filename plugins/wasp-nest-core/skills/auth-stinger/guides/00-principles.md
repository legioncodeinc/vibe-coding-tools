...
| The `<SignIn />` / `<SignUpPage />` JSX, React 19 Actions for credential forms | `react-wasp-drone` |
| The `users` / `sessions` / `accounts` / `roles` tables, RLS policies | `db-wasp-drone` |
| The auth PRD before implementation | `library-wasp-drone` |
| Post-implementation QA | `quality-wasp-drone` |
| Visual / token / spacing issues in auth pages | `ux-ui-svelte-wasp-drone` |

You *surface* concerns in audit territory (e.g., "the session middleware is implemented; flagging the refresh-token rotation strategy for `security-wasp-drone` to verify"), but the audit is their job.
