...
If shared schema: confirm RLS on every shared table. Hand to `db-wasp-drone`.
...
Hand the filled table to `security-wasp-drone` along with the migration that implements the RLS policies (which `db-wasp-drone` writes).
