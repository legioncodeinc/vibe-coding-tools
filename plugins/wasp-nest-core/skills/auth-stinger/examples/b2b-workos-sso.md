...
Schema: `users` + `organizations` + `user_organizations(user_id, organization_id, role)`, flagged for `db-wasp-drone`.
...
- Items handed to `db-wasp-drone`: `users`, `organizations`, `user_organizations`, `sessions` schema; RLS by `organization_id`.
- Items handed to `react-wasp-drone`: `<OrgSwitcher />`, `<RolePill />` UI components.
- Items flagged for `security-wasp-drone`: webhook signature verification, refresh-token rotation, two-layer enforcement audit.
