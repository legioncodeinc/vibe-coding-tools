...
The `<SignIn />` JSX placement and theming is `react-wasp-drone`'s territory; auth-wasp-drone writes the spec ("centered on a card with logo top, social-buttons-first ordering, brand color from token `--color-primary`") and `react-wasp-drone` writes the layout.
...
The `users` schema is **flagged for `db-wasp-drone`**. auth-wasp-drone doesn't write the migration.
...
- Items handed to `db-wasp-drone` (schema), `react-wasp-drone` (UI), `quality-wasp-drone` (post-implementation QA).
- Items flagged for `security-wasp-drone`: webhook signature verification config, the unused-client-deletion synthetic-call schedule.
