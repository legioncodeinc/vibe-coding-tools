...
export const auth = betterAuth({
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url }) => {
      // Wire to email provider. No Drone owns email delivery yet; see the gap list.
...
### 2. Schema (flag for db-wasp-drone)

```bash
npx @better-auth/cli@latest generate
```

Generates: `user`, `session`, `account`, `verification`, `passkey`, `two_factor`, `organization`, `member`, `invitation` tables. Hand the migration file to `db-wasp-drone`. db-wasp-drone owns indexing, FK ON DELETE behavior, RLS if needed.
...
### 5. Custom UI (spec for react-wasp-drone)

auth-wasp-drone writes the spec, react-wasp-drone implements:
...
`react-wasp-drone` owns the JSX wiring. auth-wasp-drone owns the auth client config and the protocol layer.
...
- Data layer: every query filters by `organization_id IN (user's orgs)`. RLS at Postgres level, flagged for `db-wasp-drone`.
...
- Items handed to `db-wasp-drone`: full migration + RLS policies.
- Items handed to `react-wasp-drone`: SignInPage / SignUpPage / OrgSwitcher specs.
- Items flagged for `security-wasp-drone`:
