...
description: "Choose and configure a SaaS support stack. Use for inboxes, AI deflection, SLAs, routing, or founder triage. Read README.md for the guide map."
---

# customer-support-tooling Stinger

Arsenal for `customer-support-tooling-wasp-drone`. Encodes the decision trees, comparison matrices, configuration templates, and integration wiring guides the Angel reads when invoked.
...
Do NOT load for:
- Chat widget installation and HMAC verification code → `live-chat-support-wasp-drone`
- Authentication and SSO for the support tool → `auth-wasp-drone`
- GDPR conversation-history retention audits → `security-wasp-drone`
- Billing/subscription issues surfaced through support → `payments-wasp-drone`
...
- **Route GDPR deletion requests and data-export concerns to security-wasp-drone.** Conversation-history retention is a data-sovereignty concern that crosses into legal liability.
...
*Forged by `stinger-forge` from `customer-support-tooling-wasp-drone-command-brief.md` and `research/`. Part of the Legion AI Tools Factory by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
