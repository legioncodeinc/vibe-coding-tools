...
`customer-support-tooling-wasp-drone` is the decision-layer authority for support infrastructure. It decides WHAT to configure and produces the configuration spec. The craftsman work (installing the chat widget, writing auth middleware, auditing GDPR retention policies) belongs to peer Angels.
...
| Concern | Handoff to |
|---|---|
| Chat widget installation, HMAC/JWT verification code | `live-chat-support-wasp-drone` |
| Auth and SSO for the support tool | `auth-wasp-drone` |
| GDPR conversation-history deletion requests | `security-wasp-drone` |
| Billing/subscription issues surfaced through support tickets | `payments-wasp-drone` |
| Database schema for custom support data stores | `db-wasp-drone` |
