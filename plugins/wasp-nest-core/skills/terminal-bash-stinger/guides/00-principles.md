...
Core rules that govern every invocation of `terminal-bash-wasp-drone`. Read before any other guide.
...
When the terminal context is a Docker container, Kubernetes init container, or CI runner image, the appropriate owner is `ci-release-wasp-drone`, not `terminal-bash-wasp-drone`. The difference:

- **Workstation dotfiles:** terminal-bash-wasp-drone
- **CI step that runs `npm ci` in a GitHub Actions job:** ci-release-wasp-drone
- **Shell script that publishes the npm package:** ci-release-wasp-drone (even if the script is Bash)
