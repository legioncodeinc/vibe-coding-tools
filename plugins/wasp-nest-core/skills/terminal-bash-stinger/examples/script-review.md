...
A worked example of `terminal-bash-wasp-drone` reviewing a shell script that builds the Wasp Nestmind package and syncs the smoke-test to a staging host.
...
This script runs on a staging host. For CI-triggered release sync, `ci-release-wasp-drone` should review the CI step that calls this script. `terminal-bash-wasp-drone` covers the script's shell safety; `ci-release-wasp-drone` covers the CI integration.
...
- **Escalation:** recommend `ci-release-wasp-drone` reviews the CI step that invokes this script.
