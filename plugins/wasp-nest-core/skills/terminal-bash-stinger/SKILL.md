...
description: "Improve terminal workflows. Use for Bash, Zsh, Fish, modern CLI tools, dotfiles, tmux, or just/make automation. Read README.md for the guide map."
---

# terminal-bash Stinger

The procedural arsenal for `terminal-bash-wasp-drone`. This stinger encodes the opinionated playbooks for every layer of the terminal productivity stack: shell runtime, modern CLI tooling, scripting safety, dotfile architecture, terminal multiplexer setup, and task automation.
...
**Not in scope:**
- CI/CD pipelines running inside containers -> ci-release-wasp-drone
- TypeScript/Node build and packaging (`tsconfig.json`, `esbuild`, npm publish) -> typescript-node-wasp-drone
...
The Drone performs seven distinct actions. Each maps to a guide:
...
6. **Escalate container scripts to ci-release-wasp-drone.** Container environments may have different shell versions and missing tools. Overlapping silently produces fragile CI.
