...
## Boundary with peer Drones

This pair (`mcp-protocol-stinger` / `mcp-protocol-wasp-drone`) owns MCP protocol correctness: tool/resource/prompt contract shape, JSON-RPC framing, zod input schemas, the two-channel error model, capability negotiation, transport choice, authentication patterns, testing, and per-harness registration. It does **not** own:

| Concern | Owner |
|---|---|
| Credential/token storage hardening, OAuth flow implementation details beyond MCP's own spec | `security-wasp-drone` |
| TLS termination / process sandboxing / where a server process actually runs in production | `ci-release-wasp-drone` |
| Injection-safe query building inside a tool handler (OWASP-level) | `security-wasp-drone` (flag here; hand off) |
| Backend datastore query semantics, schema, or search internals behind a tool | the domain-specific data-layer Drone for that backend (e.g. a vector-store or database wasp-drone) |
| Documenting a confirmed-correct tool contract for consumers (reference docs, changelogs) | `mcp-tool-docs-wasp-drone` |
