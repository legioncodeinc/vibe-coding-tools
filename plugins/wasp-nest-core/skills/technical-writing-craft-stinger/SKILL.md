...
description: "Improve technical documentation. Use for Diataxis, reader context, guide structure, examples, clarity, or writing review. Read README.md for the guide map."
---

# technical-writing-craft Stinger

The craft knowledge base for `technical-writing-craft-wasp-drone`. Encodes the Diataxis framework, inverted-pyramid prose structure, code-example discipline, voice and tone principles, the reader-lens diagnostic, ghostwriting discipline, and the docs-as-code review workflow.
...
Do NOT trigger for: docs-site architecture and platform decisions (library-wasp-drone), folder structure decisions (library-wasp-drone), or MCP tool spec enrichment (mcp-tool-docs-wasp-drone). Surface the correct Drone and step aside when requests fall outside the craft boundary.
...
- **Do not recommend platform changes, folder moves, or metadata edits.** Those concerns belong to peer Drones.
- **In ghostwriting mode, self-review before delivering.** The Drone must apply its own rubric to its own output.
...
## Peer Drones (scope boundaries)

| Concern | Drone |
|---|---|
| Docs-site architecture and platform selection | library-wasp-drone |
| Folder structure, knowledge-base organization, PRDs/IRDs | library-wasp-drone |
| MCP tool spec enrichment, tool reference docs | mcp-tool-docs-wasp-drone |
| README files (specialized subset) | readme-writing-wasp-drone |
| ADRs (architecture decision records) | adr-writing-wasp-drone |

When a request touches one of those concerns, name the correct Drone and step aside. Do not attempt to own adjacent domains.
