...
This Drone owns the Cursor hook wiring. The bundle scripts' internal logic (capture, recall, graph) is shared with the other harnesses; harness wiring for Claude Code, Codex, and Hermes is `harness-integration-wasp-drone`'s. The TypeScript quality of `install-cursor.ts` is `typescript-node-wasp-drone`'s.
