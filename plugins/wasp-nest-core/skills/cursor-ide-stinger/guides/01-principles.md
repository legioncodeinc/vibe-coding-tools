...
The mental model for `cursor-ide-wasp-drone`. Read this before any rule, MCP, hook, layout, or extension work.

## The Wasp Nestmind Cursor surface
...
4. **The `.cursor/` colony.** The rules (`.mdc`), agents (`*.md`), skills/Stingers, commands (`the-pest-controller`, `the-smoker`), and `model-comparison-matrix.md` that make the colony run inside Cursor. Authoring rules is `guides/02`; the layout is `guides/05`.

Everything this Drone does lives in one of those four. If a task is about the TypeScript quality, the MCP tool schemas, or another agent's harness, it belongs to a different Drone (see "When to defer" below).
...
`.cursor/rules/no-em-dashes.mdc` (alwaysApply) bans em dashes (`U+2014`) and en dashes (`U+2013`) in any prose authored on the user's behalf: chat, docs, commit messages, comments, every file this Drone writes. Use a comma, colon, parentheses, or a period instead. Write hyphens directly; do not run a blanket replace that could corrupt code.

## When to defer to other Drones

`cursor-ide-wasp-drone` owns the Cursor configuration and platform layer. Hand off when:

- The TypeScript quality or typing of `install-cursor.ts` / the extension source is the concern -> `typescript-node-wasp-drone`.
- The MCP server's tool schemas, Zod validation, or transport is the concern -> `mcp-protocol-wasp-drone`.
- The harness for Claude Code, Codex, or Hermes is the concern -> `harness-integration-wasp-drone`.
- TypeScript/UI code inside the extension's webview -> `typescript-node-wasp-drone`.
- Publishing or CI for the extension -> `ci-release-wasp-drone`.
- Every implementation task closes out with `security-wasp-drone` first, then `quality-wasp-drone`.
