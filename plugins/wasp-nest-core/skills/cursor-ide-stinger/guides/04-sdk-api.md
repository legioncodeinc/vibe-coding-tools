...
Providing the SDK code that runs agents is `cursor-ide-wasp-drone`'s job. Wiring it into GitHub Actions or Docker containers is `devops-wasp-drone`'s job. After writing the SDK script, hand off to `devops-wasp-drone` for pipeline integration.
