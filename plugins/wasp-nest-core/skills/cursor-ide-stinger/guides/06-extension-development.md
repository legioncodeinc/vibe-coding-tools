...
This Drone owns the extension's manifest, contributions, build, and the `vscode.*` activation surface. The TypeScript/UI code inside the dashboard webview and the TypeScript quality of the extension source are `typescript-node-wasp-drone`'s. Packaging/publishing and CI are `ci-release-wasp-drone`'s.
