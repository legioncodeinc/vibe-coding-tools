# Guide 03: Registering the Wasp Nestmind MCP Server in Cursor
...
The tool definitions, Zod schemas, and transport are owned by `mcp-protocol-wasp-drone`. This Drone only wires the server into Cursor.
...
The Wasp Nestmind MCP server is a stdio server. Point Cursor at the built entrypoint:
...
By default Cursor asks before calling any MCP tool. To let the agent call the Wasp Nestmind tools without prompting: Cursor Settings > MCP > "Allow Agent to run tools without asking" (enable per-tool or globally). The Wasp Nestmind tools are read-only recall, so per-tool auto-approval is reasonable.
...
Registering the server in Cursor is this Drone's job. The server's tool schemas, search logic, and transport are `mcp-protocol-wasp-drone`'s. Harness wiring for other agents (Hermes registers these same tools) is `harness-integration-wasp-drone`'s.
