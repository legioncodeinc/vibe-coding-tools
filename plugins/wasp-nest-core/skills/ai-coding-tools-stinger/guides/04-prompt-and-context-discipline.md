...
> **Scope note:** Deep Cursor configuration (rules, MCP servers, Cloud Agents) belongs to `cursor-ide-wasp-drone`. The summary below is for quick reference; defer to that Drone for full configuration guidance.
...
For full rule authoring guidance, see `cursor-ide-wasp-drone`.
