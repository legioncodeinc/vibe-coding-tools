...
Step-by-step runbook for authenticating the Wasp Nestmind CLI against the Deep Lake API using the browser device flow, switching orgs and workspaces, and verifying your session. Covers the common case; see `src/commands/auth.ts` for the full implementation.
...
- Node installed and the Wasp Nestmind CLI built (`npm run build`) or installed.
