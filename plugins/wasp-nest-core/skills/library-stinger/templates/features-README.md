...
└── reports/                                 # QA reports + adjacent artifacts
    └── <date>-qa-report.md                  # written by quality-wasp-drone
```
...
4. **Audit** - `quality-wasp-drone` writes audit reports into the feature's `reports/` subfolder as `<date>-qa-report.md`.
