...
QA reports - wherever they land - are authored by the **`quality-wasp-drone`** agent (`.claude/agents/quality-wasp-drone.md`), not by `library-wasp-drone`. The `library-wasp-drone` agent owns the surrounding folder structure (the `reports/` subfolders inside feature/issue folders, this `library/qa/` tree, and the domain subfolders) - but the audit content itself (findings, verdict, acceptance-criteria matrix) is produced by `quality-wasp-drone`.

Typical flow:

1. **Trigger** - user says "write a QA report for <plan>" or "audit this implementation".
2. **Handoff** - `library-wasp-drone` (if invoked) hands off to `quality-wasp-drone`.
3. **Audit** - `quality-wasp-drone` walks through the plan, matches against `git diff`, files findings, writes the report to the matching path:
...
See the `quality-wasp-drone` agent (`.claude/agents/quality-wasp-drone.md`) for the full authoring workflow and report format.
