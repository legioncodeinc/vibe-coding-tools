...
description: "Design a React icon system. Use for library choice, bundle size, SVG sprites, dynamic icons, or accessibility. Read README.md for the guide map."
---

# icon-system Stinger

Procedural arsenal for `icon-system-wasp-drone`. Encodes the canonical icon library selection matrix, the tree-shake-vs-SVG-sprite delivery decision, the dynamic-import-by-name pattern, custom SVG component conventions, and the accessibility contract covering the three icon categories (decorative, semantic, interactive).

---

## When this stinger applies

Load when `icon-system-wasp-drone` is invoked. Typical triggers:
...
- Icon size and color tokens (ux-ui-svelte-wasp-drone owns those)
- General React component architecture (react-wasp-drone)
- SVG sprite build-pipeline tooling at the bundler level (devops-wasp-drone)
- General bundle-optimization strategies beyond icon imports (devops-wasp-drone)
...
*Forged by stinger-forge from `icon-system-wasp-drone-command-brief.md` and `research/`. Part of The Wasp Nest by [Mario Aldayuz a.k.a @thenotoriousllama](https://github.com/thenotoriousllama).*
