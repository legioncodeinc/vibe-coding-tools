...
Invoked `seo-aeo-wasp-drone` with: "Run Phase 3 on `apps/web`. Framework: SvelteKit. Create: generateSEO.ts, schema.ts, sitemap +server.ts, robots +server.ts, <svelte:head> patterns for blog routes."
...
## Downstream Drones recommended

- `seo-aeo-wasp-drone`: production SEO audit + schema validation
- `security-wasp-drone`: CSP tightening review
- `quality-wasp-drone`: implementation-vs-PRD verification pass
