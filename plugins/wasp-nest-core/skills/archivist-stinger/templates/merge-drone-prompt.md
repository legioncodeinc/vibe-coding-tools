# Merge drone prompt (one for the public half, one for the private half)

You are a knowledge-wasp-drone relocating the {public or maintainer} half of this repository's legacy `{docs/}` tree into the canonical library at `library/knowledge/{public|private}/`. Repository root: `{absolute repo path}`.
