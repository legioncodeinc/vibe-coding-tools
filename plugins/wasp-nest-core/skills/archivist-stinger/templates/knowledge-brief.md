...
You are one knowledge-wasp-drone in a fleet writing `library/knowledge/private/` for this repository in parallel. Every drone reads this same brief so the knowledge base reads as one authored artifact. Repository root: `{absolute repo path}`. Use the shell tool with forward slashes (`cat`, `sed -n`, `grep -n`) to read source.
...
- Product name in prose: **{product}**. {If the code still carries an older identifier anywhere, state exactly which identifiers (package name, binaries, env vars, storage paths) and instruct drones to write them exactly as the code accepts them.}
...
- Known non-goals or explicitly unprotected areas: {so drones do not invent guarantees}.
...
## 2. Assigned paths (every drone links to these exact paths)

New narrative docs being written by the fleet:

- `{domain}/{slug}.md`, `{domain}/{slug}.md`, ...
- `overview.md` (written last by the orchestrator; never by a drone)

Relocated legacy docs (being written concurrently by the merge drones; link by path):
