...
Phase 4. The private knowledge base (`library/knowledge/private/`) is written by a fleet of `knowledge-wasp-drone` instances, one or two per domain, from a single shared brief. This phase is executed by the orchestrator, not by `archivist-wasp-drone`, because the Wasp Nest forbids nested Drone spawns: the archivist prepares the brief and the assignments and hands them back; the orchestrator dispatches the fleet and re-dispatches the archivist for the closing pass.
...
5. **Write one assignment per drone** from the template: the folder, the docs with required content and the exact symbols and files to read, the diagram each doc needs, and the sibling files another drone owns.
...
1. **Probe the model alias with one drone** before fanning out; an unmapped alias rejects every spawn at once.
2. **Baseline the tree** excluding `library/` so writer discipline can be proven afterwards.
3. **Spawn every drone at top level**, armed with `knowledge-stinger`, in the background, with the assignment prompt. Two drones on the priority domain, one per ordinary domain, one for the small domains combined.
...
8. **Re-dispatch `archivist-wasp-drone`** for phase 5 and the report.
