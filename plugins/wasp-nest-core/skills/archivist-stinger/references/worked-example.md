...
| Merge | Two merge drones in parallel: public half (90 files, release notes scripted) and private half (42 headed docs plus 27 verbatim evidence files, hash-verified) | 160 legacy files, 0 unplaced |
| Fleet | Twelve writer drones across thirteen domains; two drones on the priority domain; one shared brief file; every writer reported line counts, sources read, and unverified facts | 43 new docs, 12,000 lines |
...
1. **Model alias mapping is a harness property, not a model property.** Eleven writers launched on the `opus` alias were rejected at spawn with a nonexistent Codex model id. Nothing was written, so nothing was lost, but the relaunch cost a full turn. Probe one drone first.
...
5. **Merge drones leave scraps.** One writer left a 5 KB scratch file at the repository root. The tamper baseline caught it; the boundary rule kept it out of the commit until the user could decide.
...
- Merge: two drones, both under 20 minutes wall clock, 0 unplaced files, 1 cross-half link wrong (an audit filed in a different folder by each drone; fixed in the final pass).
