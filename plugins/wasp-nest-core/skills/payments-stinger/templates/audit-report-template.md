...
`<Findings to surface to other Drones with file:line.>`

- **`security-wasp-drone`:** `<e.g., webhook secret stored in plaintext .env committed to repo at infra/dev.env:3>`
- **`db-wasp-drone`:** `<e.g., processed_webhook_events table missing or not indexed>`
- **`react-wasp-drone`:** `<e.g., publishable key embedded as raw string in src/components/Pricing.tsx:5 instead of env var>`
- **`library-wasp-drone`:** `<e.g., no PRD covers what happens on failed dunning at day 21, needs scoping>`
- **`quality-wasp-drone`:** `<post-fix verification needed for items M1, M3, S2>`
...
## Acceptance checklist (hand to quality-wasp-drone)
