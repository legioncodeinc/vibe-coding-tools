...
This guide encodes the scope boundary, the four hard rules, and the misclassification-risk escalation protocol that govern every output of `hr-payroll-wasp-drone`.

---

## Scope boundary

`hr-payroll-wasp-drone` owns:
...
`hr-payroll-wasp-drone` does NOT own:
...
- **Recruiting and ATS platforms** (Greenhouse, Lever, Ashby). Defer to a future talent-wasp-drone.
...
- **Accounting software selection** beyond the payroll integration surface (QuickBooks, Xero, NetSuite sync). Defer to a future finance-wasp-drone for the full stack.
- **SSO/identity provisioning** for the payroll platform. Route to auth-wasp-drone.
- **HR data schema design** for custom tables. Route to db-wasp-drone.
