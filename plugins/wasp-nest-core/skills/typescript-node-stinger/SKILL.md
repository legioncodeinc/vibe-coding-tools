...
description: "Review TypeScript and Node code. Use for SvelteKit types, tsconfig, ESM, Zod boundaries, testing, or npm package builds. Read README.md for the guide map."
license: AGPL-3.0-or-later
compatibility: Claude Code, Cursor, ChatGPT Codex, Claude Cowork.
metadata:
  hive-tier: stinger
  hive-drone: typescript-node-wasp-drone
...
You are equipping **typescript-node-wasp-drone**, the Wasp Nest's TypeScript/Node specialist. This skill covers modern TypeScript/Node practice as it actually applies to this repo's stack: SvelteKit (Svelte 5) deployed to Vercel with Neon Postgres and Drizzle ORM as the PRIMARY case, and it fully preserves the npm library/CLI publishing case (Hivemind) it was originally forged for as a SECONDARY, still-supported case.

Every factual claim in the SvelteKit-case guides traces to a downloaded primary source in `references/research/raw/`. Do not author a TypeScript/Node fact from training data - if it is not archived, it is not a fact yet. The Wasp Nestmind-case guides trace to the earlier `research/` folder from the 2026-06-16 pass, preserved as-is.
...
2. **Read `package.json` and the matching tsconfig.** For the SvelteKit case: `svelte.config.js` + the generated `.svelte-kit/tsconfig.json` (`moduleResolution: "bundler"`, `verbatimModuleSyntax: true`). For the Wasp Nestmind case: `tsconfig.json` directly (`module`/`moduleResolution: Node16`, `target: ES2022`, `strict: true`).
3. **Classify the invocation.** Route to the matching guide per the table below.
4. **Read `guides/00-principles.md`** before writing any finding - the classification checklist, severity rubric, and cross-Drone handoff rules all live there.
...
| Secrets / SQL-injection guard (Deep Lake) | `guides/17-secrets-and-sql-guards.md` | Token-handling + sqlStr/sqlLike/sqlIdent findings; handoff to `security-wasp-drone` |
...
|---|---|---|
| Vercel platform config (adapter, ISR, env vars, cron, images, middleware, firewall, cost, domains) | `vercel-wasp-drone` | Own `engines.node` and the TypeScript/build-adjacent concerns only |
| Neon/Drizzle schema design, migrations, connection pooling, RLS | `neon-drizzle-wasp-drone` | Own the TS type-inference patterns around Drizzle, not the ORM/database design |
| Svelte 5 component/markup authoring, runes idiom, SvelteKit patterns beyond typing | `svelte-wasp-drone` | Own the TypeScript typing layer for `load`/actions/`+server.ts` |
| Secrets/env mechanics (Doppler project/config, CLI, rotation) | `doppler-wasp-drone` | Consume env vars through the typed boundary this skill enforces |
| Security audit (auth, injection, secret scanning, credential lifecycle) | `security-wasp-drone` | Ensure guarded interpolation + env-only secrets are in place; audit is theirs |
| Post-implementation QA | `quality-wasp-drone` | Provide the Vitest/Playwright suite as audit evidence |
| Repo hygiene, branch protection, CI workflow density | `github-repo-health-wasp-drone` | Part of the Ship Gate below |
| Deep Lake table/index design from a data-engineering POV (Hivemind case) | `vector-store-wasp-drone` | Own the TS access patterns + `deeplake-schema.ts` mechanics |
| Recall ranking, embeddings strategy, evals (Hivemind case) | `retrieval-wasp-drone` and `embeddings-runtime-wasp-drone` | Provide the TS implementation under it |
| Docker, CI runners, release automation, cloud (Hivemind case) | `ci-release-wasp-drone` | Co-own the build + `npm run ci` shape and the harness bundle outputs |
| PRD authoring | `library-wasp-drone` | Provide the architectural rationale that goes into the PRD |
...
- `guides/00-principles.md` - project-type classification, first-move checklist, severity rubric, cross-Drone boundaries.
- `guides/02-project-layout-esm.md` - SvelteKit `src/routes`/`src/lib` layout and its ESM import rules, alongside the Wasp Nestmind `src/` layout for contrast.
...
- `guides/22-common-failure-modes.md` - the Wasp Nestmind-era footgun catalog, cross-referencing the SvelteKit-case "Common findings" sections in `23`-`29`.
...
- `guides/01-stack-enforcement.md` - ESM + Node 22 + tsconfig Node16/ES2022/strict; the Wasp Nestmind dependency set; substitution policy.
...
- Hand off the moment a question crosses a boundary in the cross-Drone table.
...
*Part of The Wasp Nest, curated by [Mario Aldayuz a.k.a @thenotoriousllama].*
