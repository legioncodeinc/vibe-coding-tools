...
A hand-written type that duplicates a table's shape instead of using `$inferSelect`/`$inferInsert` is a **must-fix** - it's the Drizzle-specific instance of this skill's general "no duplicate source of truth for a data shape" principle (the same reasoning behind the Wasp Nestmind case's single-sourced Deep Lake schema, `guides/15-deeplake-schema-healing.md`).
