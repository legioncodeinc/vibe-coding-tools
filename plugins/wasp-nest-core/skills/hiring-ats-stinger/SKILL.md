...
description: "Choose and configure an applicant tracking system. Use for pipeline stages, scorecards, calibration, sourcing, or ATS-to-HRIS handoff. Read README.md for the guide map."
...
- Escalate PII/GDPR questions to `security-wasp-drone`. Candidate data is PII; GDPR right-to-erasure for applicants is outside this stinger's scope.
...
- Escalate HRIS configuration depth (beyond the ATS handoff interface) to a future `hris-wasp-drone`.
...
- When `hiring-ats-wasp-drone` is invoked by the orchestrator
