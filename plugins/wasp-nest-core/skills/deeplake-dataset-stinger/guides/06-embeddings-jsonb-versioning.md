...
Three intertwined storage decisions: how embeddings are stored (`FLOAT4[]`), how schemaless payloads are stored (JSONB `message`), and how edits are recorded (append-only version-bump). deeplake-dataset-wasp-drone owns the storage shape; retrieval is `retrieval-wasp-drone`.
...
`<#>` cosine distance, optionally combined with BM25 via hybrid. The operator choice is `guides/02-indexing.md`. deeplake-dataset-wasp-drone picks the column shape and the operator, then hands chunking / reranking / eval to `retrieval-wasp-drone`.
...
- Hand off retrieval / recall to `retrieval-wasp-drone`.
