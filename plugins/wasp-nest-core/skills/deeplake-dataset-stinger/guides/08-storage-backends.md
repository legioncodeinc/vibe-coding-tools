...
Deep Lake datasets are bring-your-own-cloud. The same dataset can live on Activeloop-managed storage, your own object store, or local / in-memory. deeplake-dataset-wasp-drone picks the backend and the credential model; `security-wasp-drone` audits the secrets.
