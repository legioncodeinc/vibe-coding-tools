...
- [ ] Roles/permissions are enforced in **both** the route/hooks layer and the data-access layer (RLS or an explicit org-scoped filter) - a single JWT check is a single point of failure (Wasp Nest convention, see `references/rbac-model.md`).
