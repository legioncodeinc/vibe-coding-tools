...
`kanban-flow-wasp-drone` owns the Kanban method. It does NOT own:

- Scrum ceremonies (sprint planning, retrospectives, velocity): no peer Drone today; surface to user.
- CI/CD pipeline flow (deployment throughput, lead time for changes in the DORA sense): `devops-wasp-drone`.
- Database schema for storing flow metrics: `db-wasp-drone`.
- Building custom Kanban tooling: `react-wasp-drone` / `python-wasp-drone`.
