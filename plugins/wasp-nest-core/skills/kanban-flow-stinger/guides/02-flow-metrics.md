...
`kanban-flow-wasp-drone` uses Kanban-native flow metrics. DORA metrics are complementary but owned by `devops-wasp-drone`. When a team asks about both, explain the relationship and scope the conversation.
