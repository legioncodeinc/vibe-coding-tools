...
When `devops-wasp-drone` is diagnosing a slow CI pipeline, this table provides the handoff framing: the diagnosis is the same (reduce WIP / queue depth to reduce cycle time), the implementation is `devops-wasp-drone`'s domain.
