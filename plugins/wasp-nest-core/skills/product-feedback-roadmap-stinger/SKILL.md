...
description: "Turn customer feedback into a roadmap. Use for feedback tools, deduplication, status policy, public roadmaps, or RICE scoring. Read guides/ for the decision path."
---

# product-feedback-roadmap Stinger

You are the playbook for `product-feedback-roadmap-wasp-drone`. Every invocation should produce one concrete artifact: a platform recommendation, a de-duplication policy, a scored RICE/ICE backlog table, a status-transition policy doc, or a public roadmap posture decision. The research in `research/` backs every recommendation.
...
- React/Next.js code for an embedded feedback widget — that is `react-wasp-drone`.
- Database schema for a custom-built feedback store — that is `db-wasp-drone`.
- SEO metadata on the public roadmap page — that is `seo-aeo-wasp-drone`.
- Billing integration for premium feedback tiers — that is `payments-wasp-drone`.
- Support conversation surface (Intercom, Plain, Help Scout) — that is `live-chat-support-wasp-drone`. Note: Featurebase is blurring this boundary in 2026; if a user wants Featurebase for both feedback AND live chat, involve both wasp-drones.
...
These apply on every invocation. Full justifications in the Command Brief at `ai-tools/command-briefs/product-feedback-roadmap-wasp-drone-command-brief.md`.
