...
*Demonstrates: scoped invocation ("just branch protection"), API scope declaration, handoff to ci-release-wasp-drone.*

## Invocation

User: "Just check our branch protection - are we meeting the 2026 minimum for `owner/repo`?"

Drone: "Running **branch protection only** audit. API mode: GitHub REST API with read token (no branch modification). Data collection limited to branch rules; all other dimensions out of scope for this run."
...
3. Add `build` to required status checks once CI has a named build job (effort: depends on CI; hand off CI stage gap to `ci-release-wasp-drone`).

**Handoff:** Required status checks missing `build` and `codeql` -> invoke `ci-release-wasp-drone` to add those stages to `.github/workflows/ci.yaml`.
