...
Not your call. The plan is the source of truth. Note the divergence in the Notes column of the traceability table and escalate to `library-wasp-drone` in the Summary. Do not silently re-classify.

### "Security issue, but `security-wasp-drone` should have caught this"

If `security-wasp-drone` ran and missed it, flag as Critical under Detrimental Patterns and note in the Summary that `security-wasp-drone` should be re-run. Do not bury it.
