...
If the user's message includes a path (e.g., "QA `library/requirements/<lifecycle>/prd-007-search/prd-feature-007-search.md`"), use that file directly. Do not second-guess the choice. Verify the file exists and is readable.
...
```bash
ls -la library/requirements/<lifecycle>/ 2>/dev/null
ls -la library/issues/<lifecycle>/ 2>/dev/null
...
Canonical convention used by `library-wasp-drone`:

- Features: `library/requirements/<lifecycle>/prd-<###>-<title>/prd-feature-<###>-<title>.md` (or `prd-feature-<###>-<title>-ck-<clickupId>.md` when sourced from ClickUp). Completed feature folders move to `library/requirements/<lifecycle>/completed/`.
- Issues: `library/issues/<lifecycle>/ird-<###>-<title>/ird-issue-<###>-<title>.md`.

Older repos that have not yet adopted this convention may keep plans in `docs/prd/`, `docs/plans/`, `tasks/`, `.specify/specs/`, or a root-level `SPEC.md`. Accept any of these and flag the drift to `library-wasp-drone` separately.
...
```bash
git log --oneline --all -- 'library/requirements/<lifecycle>/**/feature-*.md' | head -20
git log --oneline --all -- 'library/issues/<lifecycle>/**/issue-*.md' | head -20
```
...
> "I couldn't locate the plan document. Is it in `library/requirements/<lifecycle>/`, `library/issues/<lifecycle>/`, or somewhere else? If you don't have one, I need at least a short spec, the audit depends on comparing the implementation against stated requirements."
...
- Produce **one report per plan**, each in that plan's own `reports/` subfolder as `<date>-qa-report.md` (or in `library/requirements/reports/<domain>/` for standalone audits).
...
- This is not a plan audit. Decline politely and recommend either:
  - Having `library-wasp-drone` author a backwards-PRD from the diff first, then running QA against it, or
  - Running a plan-agnostic code review (which is not this Drone's job, suggest the `review` skill or a generic AI code reviewer like CodeRabbit).
...
- Example: `examples/01-happy-path-clean-audit.md` shows the plan location as `library/requirements/<lifecycle>/prd-007-user-profile-badge/prd-feature-007-user-profile-badge.md` and the report reflects that in its heading.
