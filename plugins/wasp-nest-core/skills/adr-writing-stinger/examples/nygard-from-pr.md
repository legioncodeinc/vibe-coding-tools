...
>
> We keep letting unsafe tool calls reach wasp drones before any check runs. This PR adds a pre-tool-use gate that inspects the serialized tool-call string and blocks disallowed patterns before dispatch:
> - Pure string/regex matching, no parsing of the tool payload into an AST
> - Runs in the harness before the call ever reaches `harness-integration-wasp-drone`
...
The harness dispatched tool calls to wasp drones with no centralized safety check. Each
drone was expected to self-police, which it did inconsistently. By Q1 2026 we had three
near-misses where a malformed or disallowed tool call reached a drone before any validation
...
call reaches `harness-integration-wasp-drone`. The gate matches the serialized request
...
**Positive:**
- One centralized choke point; drones no longer self-police tool safety.
...
**Neutral:**
- Existing drones need no code change; the gate is upstream of them.
