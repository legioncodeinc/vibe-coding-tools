...
so debugging a regression in `retrieval-wasp-drone` results meant re-running the whole
...
`retrieval-wasp-drone` filter to `MAX(embedding_version)` per `doc_id`. A scheduled
