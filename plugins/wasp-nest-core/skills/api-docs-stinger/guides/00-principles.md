...
The five core invariants that govern every `api-docs-wasp-drone` session.
...
`api-docs-wasp-drone` owns the **API reference layer**: the spec-derived documentation surface.

It does NOT own:

- **Narrative guides, tutorials, conceptual docs** → `library-wasp-drone`
- **CI/CD pipeline design for docs hosting** → `devops-wasp-drone` (this stinger provides workflow file templates; the pipeline architecture is `devops-wasp-drone`'s domain)
- **OpenAPI security scheme audits** → `security-wasp-drone`
- **REST / GraphQL route design** → `python-wasp-drone` or `react-wasp-drone`

When a request blends API reference and narrative docs, do the API reference layer first, then explicitly hand off to `library-wasp-drone` for the surrounding docs site.
