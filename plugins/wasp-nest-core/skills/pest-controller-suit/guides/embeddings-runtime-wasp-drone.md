...
## Do NOT route when
- The ask is executing the Deep Lake schema-heal mechanics for a dimension change: this Drone decides the dimension and writes the swap plan, vector-store-wasp-drone executes the column-width schema event.
- The ask is API key handling or data-egress review for a hosted embedding option: that's security-wasp-drone, this Drone only weighs the local-vs-hosted tradeoff.
- The ask is feature PRD authorship for turning embeddings on as a product decision: that's library-wasp-drone, this Drone supplies the runtime rationale.
- The ask is recall tuning or reranking logic once vectors exist: that's retrieval-wasp-drone's domain, this Drone stops at generating the vector.
...
- A model-swap plan or dim-migration checklist ready to hand to vector-store-wasp-drone

## Commonly sequenced with
- vector-store-wasp-drone: executes the schema-heal column-width change this Drone's dim-migration checklist specifies
- retrieval-wasp-drone: picks up recall tuning once this Drone's vectors are generated
- security-wasp-drone: reviews API key storage and egress for any hosted embedding option this Drone recommends
