...
## Do NOT route when
- The ask is typeface selection or the brand's typographic identity itself: route to design-system-wasp-drone.
- The ask is applying already-defined type tokens to a specific component: route to ux-ui-svelte-wasp-drone.
- The ask is build-pipeline font optimization such as `glyphhanger` running in CI: route to devops-wasp-drone.
- The ask is the data schema for persisted user font preferences: route to db-wasp-drone.
- The ask is the broader Core Web Vitals/LCP audit beyond `font-display` and preload strategy: route to seo-aeo-wasp-drone.
...
- design-system-wasp-drone: for the typeface/brand decision this Drone's technical implementation serves.
- ux-ui-svelte-wasp-drone: for applying the resulting type tokens at the component level.
- seo-aeo-wasp-drone: for the Core Web Vitals measurement loop once `font-display` and preload are set.
