...
## Do NOT route when
- The ask is the Mini App frontend UI/React layer rather than the bot-side initData wiring: route to react-wasp-drone.
- The ask is Docker/CI/CD for the bot server: route to devops-wasp-drone.
- The ask is an external payment processor beyond Telegram Payments/Stars: route to payments-wasp-drone.
...
- react-wasp-drone: for the Mini App's frontend UI once initData validation is wired.
- devops-wasp-drone: for deploying and running the bot server.
- payments-wasp-drone: when the product also needs a non-Telegram payment processor for physical goods.
