...
## Do NOT route when
- The ask is the SvelteKit route/component markup itself, not the data-fetching or table wiring: route to ux-ui-svelte-wasp-drone.
- The ask is Vercel deployment or caching configuration, even if it interacts with a Query prefetch pattern: route to vercel-wasp-drone (this Drone only flags the interaction).
- The ask is the underlying database schema behind a query function or table's row data: route to db-wasp-drone.
...
- Confirmation the project is Svelte 5 SvelteKit (this Drone's supported target); other frameworks get reduced coverage.
...
- ux-ui-svelte-wasp-drone: for the surrounding route/component markup the table or form renders into.
- vercel-wasp-drone: when a Query prefetch pattern interacts with Vercel's ISR/Cache-Control behavior.
- db-wasp-drone: for the data source behind a query function or table's row data.
