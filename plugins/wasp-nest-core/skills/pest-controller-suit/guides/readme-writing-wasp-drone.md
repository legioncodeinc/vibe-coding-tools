...
## Do NOT route when
- The README has grown past 2,000 words and needs a full documentation site instead: hand off to `library-wasp-drone` for docs-site architecture rather than continuing to restructure a single file.
- The task is per-entity code extraction into a wiki: route to `wiki-wasp-drone`.
- The task is CI badge pipeline wiring itself (making the CI produce the badge data): route to `devops-wasp-drone`; this Drone only decides which badges belong and how they're displayed.
- The request is for `.rst` format: route to `python-wasp-drone` for the ecosystem-specific convention.
...
- `library-wasp-drone` after: when the README exceeds 2,000 words and needs docs-site extraction
- `devops-wasp-drone` before: CI pipeline must exist before its badge can be wired into the README
- `wiki-wasp-drone` alongside: when the same effort also needs per-entity documentation extraction
