...
## Do NOT route when
- The project is Next.js App Router rather than SvelteKit: this Drone's paired Stinger was rebuilt specifically for SvelteKit and does not apply SvelteKit file conventions to a different framework; flag explicitly rather than degrading silently.
- The task is writing marketing copy or picking keywords: that's a content Drone's job, not this Drone's.
- The task is CSP or security header changes touching `hooks.server.ts` or `svelte.config.js`: route through `security-wasp-drone` for the security pass before merge.
- The task is React component architecture unrelated to metadata or rendering-for-discoverability: route to `react-wasp-drone`; this Drone surfaces SEO concerns in React-adjacent work but doesn't own component design.
...
- `security-wasp-drone` before merge: for any CSP or security-header change touching SvelteKit hooks or config
- `library-wasp-drone` after: for a large phased SEO rollout that warrants a feature PRD
- `react-wasp-drone` or Svelte UI skills alongside: coordinating on shared components that affect both design and discoverability
