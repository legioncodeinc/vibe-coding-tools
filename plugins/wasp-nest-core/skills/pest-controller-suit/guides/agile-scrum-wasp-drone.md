...
## Do NOT route when
- The request is configuring Jira, ClickUp, or Azure DevOps as tools; this Drone names it a tooling concern and stops, it does not configure the tool.
- The request is implementing CI/deployment gates that back a Definition of Done; that is devops-wasp-drone.
- The request is code review, security review, or architecture guidance; route to the domain-specific Drone.
- The framework-selection assessment clearly favors Kanban; acknowledge it and offer to route to a Kanban-specific Drone rather than keep coaching Scrum.
...
- devops-wasp-drone: implements the CI/deployment gates a Definition of Done requires.
- Tooling-configuration work (Jira/ClickUp/Azure DevOps): this Drone flags the requirement and stops, a different owner configures it.
- Domain Drones (security, code review, architecture): take over once the conversation leaves process and enters implementation.
