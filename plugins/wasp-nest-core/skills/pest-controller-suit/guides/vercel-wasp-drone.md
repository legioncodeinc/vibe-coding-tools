...
## Do NOT route when
- The ask is the SvelteKit app's own route/component logic, not the deployment configuration: route to ux-ui-svelte-wasp-drone.
- The ask is the Neon schema or migrations themselves, not the `DATABASE_URL` wiring: route to db-wasp-drone.
- The ask is TanStack library usage inside the deployed app: route to tanstack-wasp-drone.
- The ask is general non-Vercel CI/CD pipeline design: route to devops-wasp-drone.
- The ask is a security audit of the resulting deployment/firewall configuration: route to security-wasp-drone.
...
- ux-ui-svelte-wasp-drone: for the route/component logic the deployment config wraps.
- db-wasp-drone: for the Neon schema behind the `DATABASE_URL` this Drone wires.
- tanstack-wasp-drone: when a Query prefetch pattern interacts with ISR/Cache-Control behavior.
- security-wasp-drone: for auditing the resulting firewall/deployment configuration.
