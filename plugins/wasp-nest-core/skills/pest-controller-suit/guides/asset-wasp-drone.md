...
## Do NOT route when
- The request is general knowledge-base authorship outside `library/knowledge/private/asset-registry/`; that is library-wasp-drone.
- The request is QA report authorship; that is quality-wasp-drone, this Drone only flags drift.
- The request is UX-UI knowledge-base content beyond the shared tokens catalog; that is ux-ui-svelte-wasp-drone.
- The request is a security posture review of a registry feature PRD; that is security-wasp-drone.
- The asset doesn't actually exist in the codebase yet; this Drone never invents a registry row, it files an issue instead.
...
- library-wasp-drone: takes registry-shaped feature PRDs this Drone drafts and applies numbering plus invariants.
- quality-wasp-drone: audits implementations once this Drone flags drift.
- ux-ui-svelte-wasp-drone: co-owns the design-token catalog split; each references the other's catalog.
