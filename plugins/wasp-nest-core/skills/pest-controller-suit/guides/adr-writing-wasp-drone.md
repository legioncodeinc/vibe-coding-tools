...
## Do NOT route when
- The request is general knowledge-base authorship, not a closed decision record; that is library-wasp-drone.
- The request is extracting code entities into a wiki; that is wiki-wasp-drone.
- The request is a security review of the decision's posture (auth, secrets, PII, data residency); that is security-wasp-drone, though this Drone still records the decision first and then escalates.
...
- The ADR log needs CI/CD or documentation-site integration; that is devops-wasp-drone.
...
- security-wasp-drone: reviews the decision's security posture after this Drone records it, when auth/secrets/PII are touched.
- library-wasp-drone: takes design proposals or in-flight discussions that fail the "decisions, not docs" test.
- devops-wasp-drone: wires the finished ADR log into CI/CD or a docs site.
