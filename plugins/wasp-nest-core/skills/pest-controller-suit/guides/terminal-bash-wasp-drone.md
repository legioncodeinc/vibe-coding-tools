...
## Do NOT route when
- The shell script runs inside a Docker container or CI runner image: route to devops-wasp-drone.
- The task runner is for a Python project's build/test pipeline specifically: route to python-wasp-drone.
- The ask is security hardening of shell scripts running in production infrastructure: route to security-wasp-drone.
...
- devops-wasp-drone: when a reviewed script turns out to run inside CI or a container.
- python-wasp-drone: when the task automation wraps a Python project's build/test pipeline.
- security-wasp-drone: for hardening review of scripts touching production infrastructure.
