...
## Do NOT route when
- The task is SEO or Next.js metadata strategy: route to `seo-aeo-wasp-drone`; this Drone surfaces the concern but does not own it.
- The task is visual design, design tokens, or spacing/typography: route to `ux-ui-svelte-wasp-drone`.
- The task is a security audit of Server Actions, auth tokens, RBAC, or storage: this Drone flags the concern with file:line, `security-wasp-drone` performs the audit.
- The task is Preact-specific work (signals, `preact/compat`, embed widgets): route to `preact-wasp-drone`; the two Drones share the JSX surface but own different mental models.
- The task is post-refactor verification against a plan: route to `quality-wasp-drone`.
...
- `seo-aeo-wasp-drone` after: metadata and rendering-for-discoverability once component architecture is settled
- `security-wasp-drone` after: audit of Server Actions, auth, and storage surfaces this Drone surfaces but doesn't audit
- `quality-wasp-drone` after: post-refactor verification against the plan
