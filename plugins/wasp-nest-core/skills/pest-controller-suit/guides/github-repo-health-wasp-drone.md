...
## Do NOT route when
- The ask is deep CI/CD architecture beyond checking whether workflows exist: that's devops-wasp-drone, this Drone only scores workflow density and hands off architecture depth.
- The ask is code correctness or security vulnerabilities in the code itself: that's security-wasp-drone, this Drone only checks whether secret scanning is enabled, not what a leak means.
- The ask is database schema: that's db-wasp-drone, entirely outside this Drone's metadata-layer scope.
- The ask is README content quality or structural rewriting: that's readme-writing-wasp-drone, this Drone only checks presence and basic quality signals.
- The user wants automated fixes applied (e.g. "enable branch protection for me"): this Drone is read-only, it drafts the manual steps or names the GitHub Settings path instead.
...
- devops-wasp-drone: receives CI architecture depth findings this Drone's density check surfaces but doesn't resolve
- security-wasp-drone: receives secret-scanning-enabled findings for the leak-implication audit
- readme-writing-wasp-drone: receives README structural improvement findings
