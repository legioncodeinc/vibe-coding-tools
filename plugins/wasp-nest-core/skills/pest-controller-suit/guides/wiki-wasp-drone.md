...
- "@wiki-wasp-drone extract entities from this file"
- "lint the wiki pages in this chunk"
- "did this commit encode an architectural decision"

## Do NOT route when
- The ask is module narrative authorship (prose describing what a module does and why): route to library-wasp-drone.
- The ask is a QA report: route to quality-wasp-drone.
- The ask involves writing or reconciling `index.md`, `<type>/_index.md`, `log.md`, `hot.md`, or `.legion/file-hashes.json`: these are owned exclusively by The Wasp Nest VS Code extension's TypeScript driver, this Drone never touches them.
- The invocation is a vague `@`-mention with unclear scope: the Drone must ask one clarifying question and get explicit confirmation before writing anything, not proceed on an inferred scope.
...
- library-wasp-drone: the sibling Drone for per-module narrative documentation, as opposed to this Drone's atomic entity pages.
- quality-wasp-drone: for QA reports that reference entities this Drone has documented.
- The Wasp Nest VS Code extension's TypeScript driver: for global-state reconciliation after this Drone's per-page writes, especially after a `partial_scan: true` direct invocation.
