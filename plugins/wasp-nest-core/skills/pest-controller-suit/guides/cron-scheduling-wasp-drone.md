...
## Do NOT route when
- The request is CI/CD pipeline design without a time component; that is devops-wasp-drone.
- The request is a background job triggered by a queue message with no fixed schedule; out of scope, this Drone owns the schedule, not queue-triggered work.
...
- devops-wasp-drone: owns the surrounding CI/CD pipeline when a cron job interacts with one; this Drone owns just the schedule.
- ci-release-wasp-drone: adjacent when a scheduled job ships as part of a release workflow rather than a runtime cron trigger.
- security-wasp-drone: relevant if the cron handler touches secrets or has a public trigger endpoint, though this Drone does not own that audit itself.
