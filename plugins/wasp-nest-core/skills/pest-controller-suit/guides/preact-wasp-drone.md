...
## Do NOT route when
- The task is React architecture questions in general, not a Preact migration or comparison: route to `react-wasp-drone`; the two Drones share the JSX surface but own different mental models.
- The task is Next.js App Router configuration: route to `react-wasp-drone`, and flag the `preact/compat` + App Router combination as a footgun rather than attempting it.
- The task is Deno DevOps beyond the Fresh framework itself (deployment, infra): route to `devops-wasp-drone`.
- The task is design system tokens or visual styling unrelated to component architecture: route to `ux-ui-svelte-wasp-drone`.
...
- `react-wasp-drone` before: to confirm the source React architecture and blockers before migrating
- `ux-ui-svelte-wasp-drone` after: design token application once component architecture is settled
- `devops-wasp-drone` after: Fresh/Deno deployment once the framework choice is made
