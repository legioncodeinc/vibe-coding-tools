...
## Do NOT route when
- The request is security audit findings; that is security-wasp-drone.
- The request is implementation correctness at the logic level; that is python-wasp-drone or react-wasp-drone.
- The request is CI/CD pipeline setup; that is devops-wasp-drone.
- The request is branch protection configuration or enforcing a PR template at the repository-settings level; that is github-repo-health-wasp-drone.
- A review comment being coached contains an actual security finding; surface it separately to security-wasp-drone rather than just softening the tone.
...
- security-wasp-drone: takes any security finding surfaced mid-comment-coaching.
- python-wasp-drone / react-wasp-drone: own implementation correctness that this Drone's checklists point at but do not judge themselves.
- github-repo-health-wasp-drone: enforces PR templates and branch protection at the settings level that this Drone only coaches content for.
- devops-wasp-drone: owns the CI checks that a review checklist references but does not configure.
