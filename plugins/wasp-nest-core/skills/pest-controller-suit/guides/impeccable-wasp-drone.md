...
## Do NOT route when
- The request is product-specific design-system token enforcement, component-library wrapping, or accessibility compliance on an established system: that belongs to `design-system-wasp-drone` / `ux-ui-svelte-wasp-drone`.
- The request is backend-only or non-UI logic: that belongs to `react-wasp-drone`, `preact-wasp-drone`, or the relevant domain Drone.
- The request is a Lighthouse/perf-only audit: that belongs to `lighthouse-pagespeed-wasp-drone`.
- The request is Security acceptance: that belongs to `security-wasp-drone` (always before quality).
...
## Outputs
- **Context contract files** - `PRODUCT.md`, `DESIGN.md` (Stitch format), `.impeccable/design.json`, `.impeccable/surfaces/*.md`, owned and kept current by this Drone.
...
- Plan execution loop: `impeccable-wasp-drone` is the implementation Drone for frontend UI/UX/design work; it hands the final implemented state to `security-wasp-drone`, reruns affected checks after security fixes, and only then hands it to `quality-wasp-drone`.
- Design-system enforcement: `design-system-wasp-drone` / `ux-ui-svelte-wasp-drone` own product-specific token enforcement; `impeccable-wasp-drone` makes it mechanical via the DESIGN.md-drift rules and the detector gate.
...
- **Never self-grade** - the Drone iterates in bounded rounds; the user is the "happy" gate. A separate reviewer (army `quality-wasp-drone` or a fresh reader) audits the build against its direction contract promise-by-promise.
