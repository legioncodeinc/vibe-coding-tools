...
## Do NOT route when
- The ask is design tokens or animation values (the data-[state=open]/closed CSS itself); this Drone wires the state attributes, ux-ui-svelte-wasp-drone authors the animation.
- The ask is general React state management or component-tree architecture unrelated to overlays; that belongs to react-wasp-drone.
- The overlay gates a destructive, irreversible, or privilege-escalating action and has not yet been security-reviewed; that audit belongs to security-wasp-drone before this Drone declares it done.
...
- ux-ui-svelte-wasp-drone: authors the motion/animation values targeting the data-[state] attributes this Drone wires.
- react-wasp-drone: owns the broader component architecture the overlay lives inside.
- security-wasp-drone: reviews any overlay gating a destructive or privilege-escalating action.
