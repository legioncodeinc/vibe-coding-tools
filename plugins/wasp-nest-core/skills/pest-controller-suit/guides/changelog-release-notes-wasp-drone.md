...
## Do NOT route when
- The request is managing the deploy pipeline itself; that is devops-wasp-drone.
- The request is a full marketing launch campaign or landing page; that is website-wasp-drone.
...
- devops-wasp-drone: owns the deploy pipeline that produces the commits this Drone turns into entries.
- website-wasp-drone: takes over when the announcement grows into a full marketing campaign.
- branching-strategy-wasp-drone: upstream, since release/hotfix branch decisions shape when a changelog entry gets written.
