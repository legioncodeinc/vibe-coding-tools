...
- this Drone also triggers proactively when a PR touches an OpenAPI spec file

## Do NOT route when
- The request is general documentation sites or tutorials beyond the API reference; that is library-wasp-drone.
- The request is an OpenAPI security scheme audit; that is security-wasp-drone.
- The request is designing the REST/GraphQL routes themselves; that is python-wasp-drone or react-wasp-drone.
- The request is CI/CD pipeline architecture for docs hosting; this Drone provides workflow file templates but devops-wasp-drone owns the pipeline design.
...
- security-wasp-drone: audits the OpenAPI security schemes once the docs are built.
- python-wasp-drone / react-wasp-drone: own the underlying route design that the spec documents.
- devops-wasp-drone: owns the CI/CD pipeline design around the docs deployment workflow this Drone scaffolds.
