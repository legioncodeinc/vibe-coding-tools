...
## Do NOT route when
- The task is the embedding daemon or model itself (warmup, quantization, `nomic.ts`): route to `embeddings-runtime-wasp-drone`; this Drone owns how recall consumes vectors, not how they're produced.
- The task is the Deep Lake table schema or DDL (column types, index choice, schema healing): route to `vector-store-wasp-drone`; a dimension change is a schema event handed to them.
- The task is an API-key, PII, or prompt-injection audit of retrieved chunks or mined skills, or scope as a security control: this Drone flags with file:line, `security-wasp-drone` audits.
- The task is feature PRD authoring for a new recall mode or propagation policy: route to `library-wasp-drone`; this Drone provides architectural rationale only.
...
- `embeddings-runtime-wasp-drone` before: confirming the vector source is healthy before tuning how recall consumes it
- `vector-store-wasp-drone` before: any dimension or schema change needed before a recall fix lands
- `security-wasp-drone` after, then `quality-wasp-drone`: the standing close-out order on any multi-Drone retrieval job
