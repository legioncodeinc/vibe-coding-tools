...
## Do NOT route when
- The ask is monitoring/alerting infrastructure configuration (PagerDuty, OpsGenie, Datadog alerting rules) rather than integrating their output: route to devops-wasp-drone.
- The ask is designing an on-call rotation or the broader incident response process: route to devops-wasp-drone.
- The ask is writing the internal runbook for responding to an incident, not the subscriber-facing communication: route to runbook-writing-wasp-drone.
- The ask is archiving a finished post-mortem in the knowledge base: route to library-wasp-drone.
- A subscriber notification involves a security vulnerability disclosure: flag and defer to security-wasp-drone before publishing.
...
- devops-wasp-drone: for the monitoring/alerting rules the automation integration consumes.
- runbook-writing-wasp-drone: for the internal-facing incident runbook paired with the public communication.
- library-wasp-drone: to archive the post-mortem once published.
