...
## Do NOT route when
- The ask is choosing a docs platform (Starlight, Docusaurus, Mintlify) rather than the compile pipeline; that belongs to docs-site-wasp-drone.
- The ask is designing the mdx-components.tsx React component map; that belongs to react-wasp-drone.
- The sanitization audit reveals a broader XSS concern beyond rehype-sanitize/DOMPurify config, such as CSP headers or stored XSS; that belongs to security-wasp-drone.
- The ask is generating SDKs or enriching an OpenAPI spec from MDX docs; that belongs to api-docs-wasp-drone.
...
- docs-site-wasp-drone: decides the platform before this Drone configures the pipeline on top of it.
- react-wasp-drone: builds the component map this Drone's compiled output renders through.
- security-wasp-drone: takes broader XSS findings beyond sanitizer configuration.
