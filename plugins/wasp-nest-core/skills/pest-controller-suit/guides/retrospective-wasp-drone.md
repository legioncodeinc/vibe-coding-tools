...
- The task is OKR-setting: no Drone owns this domain in the Wasp Nest currently.
- A significant architectural or process decision surfaces from the retro: hand off to `library-wasp-drone` for formal documentation rather than documenting it as a retro artifact.
...
- A pointer to `library-wasp-drone` for any decision worth formal documentation

## Commonly sequenced with
- `library-wasp-drone` after: when the retro surfaces a process change or ADR-worthy decision
- Any implementing Drone before: retro action items often become follow-up work for the Drone that owns the affected domain
