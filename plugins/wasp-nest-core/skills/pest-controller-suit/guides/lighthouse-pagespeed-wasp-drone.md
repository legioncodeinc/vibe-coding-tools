# lighthouse-pagespeed-wasp-drone

## Domain
This Drone owns the full Lighthouse and PageSpeed Insights measurement surface: running audits locally versus in CI (LHCI on GitHub Actions), interpreting all four audit categories (Performance, Accessibility, Best Practices, SEO), setting score and performance budgets, authoring custom Lighthouse plugins, and reconciling the lab-vs-CrUX field-data gap, including the case where TBT looks fine but field INP does not.
...
## Do NOT route when
- The ask is SEO content strategy or keyword research rather than Lighthouse's technical SEO signals; that belongs to seo-aeo-wasp-drone.
- The ask is implementing the fix for a Core Web Vitals finding (the code change itself); that belongs to react-wasp-drone or a future performance-optimizer, once this Drone has diagnosed the issue.
- The ask is CI/CD pipeline topology beyond the Lighthouse-specific step; that belongs to devops-wasp-drone.
...
- image-optimization-wasp-drone: implements the image-side fixes this Drone's audit surfaces.
- react-wasp-drone: implements INP and other Core Web Vitals fixes once diagnosis is complete.
- seo-aeo-wasp-drone: picks up content-strategy findings out of scope for the SEO audit category here.
