...
## Do NOT route when
- The ask is HTTP/REST or MCP semantic and compatibility decisions: that is `http-rest-fundamentals-wasp-drone` or `mcp-protocol-wasp-drone`; Rust implements their approved contracts.
- The ask is product, model, and provider policy: that is `ai-tools-platform-wasp-drone` or the named product owner.
- The ask is schema architecture: that is `db-wasp-drone`; this Drone owns approved SQLx/SQLite mechanics and proof.
- The ask is Security acceptance: that is `security-wasp-drone`.
- The ask is dependency/license/advisory disposition: that is `dependency-audit-wasp-drone`.
- The ask is final implementation-to-PRD acceptance: that is `quality-wasp-drone`.
- The ask is CI/CD topology, signing, publication, or release operations: that is the appropriate DevOps or release specialist; do not use this Drone for unauthorized live credentials, paid traffic, publishing, or global installation.
- The ask is Tauri-specific window/webview integration, capabilities, permissions, plugins, sidecars, updater behavior, or bundle configuration: that is `tauri-wasp-drone`; Rust owns only the underlying Rust implementation it is assigned.

If a request straddles two Drones' domains, let the policy or protocol owner approve the contract first, then route the bounded Rust implementation to `rust-wasp-drone`.
...
If a missing input controls safety, public compatibility, money, credentials, signing, publication, or destructive behavior, invoke only to record the fail-closed blocker and smallest safe checkpoint; do not ask the Drone to guess.
...
- Plan execution loop: `rust-wasp-drone` is the implementation Drone for Rust/Cargo work; it hands the final implemented state to `security-wasp-drone`, reruns affected checks after security fixes, and only then hands it to `quality-wasp-drone`.
- Schema-touching work: `db-wasp-drone` owns schema architecture first; `rust-wasp-drone` implements approved SQLx/SQLite mechanics and proof; Security and Quality close out in their mandatory order.
- Protocol or provider work: the HTTP/MCP/platform owner establishes the contract or policy; `rust-wasp-drone` implements the bounded adapter; Security and Quality close out afterward.
- Tauri work: `tauri-wasp-drone` owns the app-shell and IPC/security integration, while `rust-wasp-drone` owns bounded Rust implementation below that contract. Each stays within assigned files before the Ship Gate.
