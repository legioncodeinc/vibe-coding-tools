...
## Do NOT route when
- The request is general Rust, Cargo, Tokio, SQLx, or service implementation unrelated to Tauri-specific integration: route to `rust-wasp-drone`.
- The request is React, Svelte, Preact, Tailwind, component, accessibility, or visual-design work beyond the typed Tauri client boundary: route to the relevant frontend or UI Drone.
- The request asks which AI model, provider, gateway, local runtime, prompt architecture, tool policy, RAG design, memory system, or evaluation approach to choose: route to `ai-tools-platform-wasp-drone` or `mind-wasp-drone`; return to this Drone to integrate the approved topology.
- The request asks for HTTP/REST or MCP method, status, header, transport, tool-schema, capability, authentication, or error semantics rather than Tauri integration of an approved protocol: route to `http-rest-fundamentals-wasp-drone` or `mcp-protocol-wasp-drone`.
- The request is the internal implementation of a Python, Node.js, or standalone Rust sidecar rather than its Tauri launch, framing, permission, lifecycle, and packaging boundary: route to the relevant language Drone. This Drone owns minimal Swift/Kotlin Tauri mobile-plugin bridge glue, but vendor native SDK internals outside that bridge return to the orchestrator or an explicitly assigned external owner because the roster has no dedicated Swift or Kotlin implementation Drone.
- The request is generic database schema, indexing, retention, or data architecture: route to `db-wasp-drone`; this Drone owns Tauri-local integration of an approved persistence contract.
- The request is formal security acceptance, dependency/advisory/license disposition, or final implementation-to-plan acceptance: route to `security-wasp-drone`, `dependency-audit-wasp-drone`, or `quality-wasp-drone`.
- The request is CI/CD topology, runner provisioning, artifact hosting, public release execution, mobile store policy/submission, signing-identity acquisition, or a live updater rollout: route to the appropriate DevOps, release, or app-store Drone. This Drone may prepare Tauri configuration and local evidence but does not authorize external effects.

If a request straddles domains, use `tauri-wasp-drone` for the Tauri-specific boundary and the narrower peer for its internal implementation or approval. For example, this Drone defines a constrained sidecar contract, then the language Drone implements the sidecar; this Drone prepares updater artifacts, then the release owner handles publication after the Ship Gate and user approval.
...
If an input controlling safety, public compatibility, money, credentials, destructive migration, signing, publication, or platform support is missing, route only to establish a fail-closed plan or smallest safe checkpoint. Do not ask the Drone to guess or perform the external effect.
...
- Rust implementation: `tauri-wasp-drone` defines the Tauri command, state, lifecycle, or plugin boundary; `rust-wasp-drone` implements general Rust mechanics underneath; Tauri-specific integration returns here for verification.
- Frontend integration: this Drone defines the typed client and trust boundary; the relevant React, Svelte, Preact, or UI Drone implements the frontend surface; this Drone verifies the IPC contract and capability scope.
- AI application work: `ai-tools-platform-wasp-drone` or `mind-wasp-drone` owns the model/provider/cognitive decision; the HTTP or MCP Drone owns protocol semantics when applicable; this Drone implements the hosted command, desktop sidecar, native runtime, or mobile plugin/bridge boundary; `rust-wasp-drone` owns general Rust internals; `security-wasp-drone` audits the composed trust boundary.
- Update or migration work: `dependency-audit-wasp-drone` disposes advisories, licenses, and lockfile risk; this Drone performs the Tauri impact review and migration; fresh implementation checks run, Security audits, affected checks rerun, and then Quality closes the implementation.
- Release work: this Drone prepares and verifies Tauri bundles, updater metadata, and signing integration locally; `devops-wasp-drone` owns pipeline and publication mechanics; `app-store-submission-wasp-drone` owns mobile store policy and submission.
- Ship Gate: implementation checks run first, then `security-wasp-drone`, then affected-check reruns, then `quality-wasp-drone`; the orchestrator loads `github-repo-health-stinger`, and the user reviews the reports before commit or push.
