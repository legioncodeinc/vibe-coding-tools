...
## Do NOT route when
- The ask is writing or reviewing documentation of a tool for consumers (name/purpose/schema/output/side-effects/examples, TypeDoc, CLI reference, changelog) rather than auditing whether the tool and its wiring are correct; that belongs to mcp-tool-docs-wasp-drone. This Drone rules on protocol correctness; mcp-tool-docs-wasp-drone transcribes that correct behavior into reference docs.
- The ask is credential storage or OAuth-token lifecycle hardening; that belongs to security-wasp-drone.
- The ask is process sandboxing or TLS for where an MCP subprocess runs; that belongs to ci-release-wasp-drone.
- The ask is backend datastore query semantics, schema, or search internals behind a tool; that belongs to the relevant data-layer wasp-drone (e.g. vector-store-wasp-drone for Hivemind's Deep Lake specifically).
...
- mcp-tool-docs-wasp-drone: documents the tool contract once this Drone confirms it is correct.
- security-wasp-drone: picks up credential/OAuth lifecycle findings surfaced during an audit.
- the relevant data-layer wasp-drone (e.g. vector-store-wasp-drone): picks up backend query/schema internals findings surfaced during an audit.
