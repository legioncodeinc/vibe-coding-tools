...
- The ask is bootstrapping a brand-new design system rather than enforcing the existing OSPRY one: route to design-system-wasp-drone.
- The ask is general Svelte language/runes questions unrelated to OSPRY enforcement ($state, $derived, $effect mechanics, SvelteKit routing not tied to a specific surface): route to svelte-wasp-drone.
- The ask is general Tailwind v4 mechanics (how `@theme` works as a mechanism, migration tooling) rather than OSPRY's specific token values: route to tailwind-wasp-drone.
- The ask is shadcn-svelte library mechanics in a project that is not one of the three OSPRY apps, or CLI/registry/anatomy questions not tied to ADR-007: route to shadcn-svelte-wasp-drone.
...
- svelte-wasp-drone: for the underlying runes/component-logic correctness before applying OSPRY-specific styling.
- tailwind-wasp-drone: for general Tailwind v4 mechanics questions that surface mid-task.
- shadcn-svelte-wasp-drone: for library-mechanics questions (CLI, registry) that aren't OSPRY-specific.
- design-system-wasp-drone: when the task turns out to be a system-level redesign rather than enforcement of the existing one.
