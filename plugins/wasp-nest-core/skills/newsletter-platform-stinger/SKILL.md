...
description: "Choose and integrate a newsletter platform. Use for Beehiiv vs Kit, signup forms, deliverability, monetization, or migration. Read README.md for the guide map."
---
...
- **Defer Stripe billing integration to `payments-wasp-drone`.** Platform-native paid tiers are in scope; custom Stripe-on-top billing is not.
...
Route infrastructure-level DNS work (SPF/DKIM/DMARC records) to `devops-wasp-drone` or the `resend` stinger.
...
*Forged by stinger-forge from `ai-tools/command-briefs/newsletter-platform-wasp-drone-command-brief.md` and `research/`. Part of the Legion AI Tools Factory.*
