...
description: "Build a web typography system. Use for variable fonts, font loading, fluid scales, vertical rhythm, or type tokens. Read README.md for the guide map."
...
|----------|------------------------|
| Font loading strategy (next/font, Fontsource, self-host) | Typeface aesthetic selection - `design-system-wasp-drone` |
| Variable font axes, subsetting, `@font-face` config | Build pipeline subsetting CI steps - `devops-wasp-drone` |
| `font-display` semantics and FOIT/FOUT/FOFT remediation | Per-component type token application - `ux-ui-svelte-wasp-drone` |
| Fluid `clamp()` type scale arithmetic | LCP font impact analysis in broader CWV audit - `seo-aeo-wasp-drone` |
| Vertical rhythm and `line-height` tokens | Persisted user font preference schema - `db-wasp-drone` |
| Font-token CSS layer (`--font-*` custom properties) | Brand identity and palette decisions - `design-system-wasp-drone` |
...
*Research trail: `research/research-summary.md` | Command Brief: `ai-tools/command-briefs/typography-font-wasp-drone-command-brief.md`*
