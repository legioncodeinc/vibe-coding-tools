...
**Architecture boundary:** This pattern requires `mind-wasp-drone` for implementation. `knowledge-base-help-center-wasp-drone` specifies the KB export format and chunking strategy inputs; `mind-wasp-drone` implements the embedding model, vector store (Qdrant), re-ranking (Cohere rerank-v3.5), and retrieval API.

**Hand-off protocol to `mind-wasp-drone`:**
...
| Custom product UI, maximum retrieval control | C: Custom RAG → hand off to `mind-wasp-drone` |
