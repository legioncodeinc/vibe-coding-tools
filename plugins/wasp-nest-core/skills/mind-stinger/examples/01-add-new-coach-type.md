...
mind-wasp-drone's job: walk the contributor through the canonical add-coach checklist.
...
- **`asset-wasp-drone`**: register `pricing_strategist` in the coach asset registry.
- **`db-wasp-drone`**: confirm the `AiCoachConfig` index strategy supports the new coach type.
- **`library-wasp-drone`**: author the PRD if this is being shipped as a feature.
