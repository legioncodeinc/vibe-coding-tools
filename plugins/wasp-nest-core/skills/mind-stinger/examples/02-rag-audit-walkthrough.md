...
- `db-wasp-drone`: query plans for the `AiTrace` aggregations were slow: possible `(tenantId, traceType, createdAt)` index needed.
- `quality-wasp-drone`: feed the eval signals into the next QA review.
