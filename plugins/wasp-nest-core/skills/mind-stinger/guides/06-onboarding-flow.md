...
| No rate limiting on the SSE endpoint | Medium | Add per-user rate limiting (handed to `security-wasp-drone`) |
| Server-side URL scraping with no sandboxing: SSRF possible | High | Add allowlist or proxy through safe external service (handed to `security-wasp-drone`) |

mind-wasp-drone flags the SSRF concern and hands the audit to `security-wasp-drone`.
