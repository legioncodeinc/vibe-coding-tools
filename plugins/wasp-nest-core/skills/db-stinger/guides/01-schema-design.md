...
For deeper audit, hand off to `security-wasp-drone`.

## PII and security flags

Mark columns that hold PII so `security-wasp-drone` can audit RLS, encryption, and retention:
...
These comments become discoverable via `pg_description`. db-wasp-drone flags PII; security-wasp-drone audits the controls.
