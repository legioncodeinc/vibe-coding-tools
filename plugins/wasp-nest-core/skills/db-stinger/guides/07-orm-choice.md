...
**db-wasp-drone's responsibility:** flag the risk in the schema/query layer with file:line, hand off to `react-wasp-drone` for the data-layer remediation (RSC vs route loader vs TanStack Query batching vs `dataloader`-style coalesce).
...
- Hand off N+1 to `react-wasp-drone`.
