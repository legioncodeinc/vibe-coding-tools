...
4. **Committed to MySQL?** → PlanetScale (db-wasp-drone flags this is non-Postgres).
...
| Caveats | **SQLite, not Postgres.** Limited concurrent writes. Different mental model. db-wasp-drone flags this. |
...
| Caveats | **MySQL, not Postgres.** No FK constraints by default (Vitess limitation, partly relaxed in 2025+). db-wasp-drone flags this. |
...
- **Supabase:** RLS is the auth boundary: security-wasp-drone audits the policies. Surface this.
