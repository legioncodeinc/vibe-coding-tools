...
Supabase has `pgvector` available out-of-the-box. db-wasp-drone picks the index family (`hnsw`) and the column type (`vector(1536)` for OpenAI `text-embedding-3-small`); **`mind-wasp-drone` owns retrieval, chunking, top-k, and reranking**.
