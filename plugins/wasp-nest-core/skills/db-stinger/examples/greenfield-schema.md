...
-- HNSW for retrieval; cosine similarity. Hand off retrieval strategy to mind-wasp-drone.
...
| `COMMENT ON COLUMN ... IS 'PII'` | `guides/01-schema-design.md` §pii-flags: surfaces to security-wasp-drone |
...
- [ ] PII columns commented for `security-wasp-drone` audit.
- [ ] Supabase RLS policies designed (handed to `security-wasp-drone`).
- [ ] PgBouncer / Supavisor in transaction mode if going serverless (`templates/pgbouncer.ini`).

## Handoffs from this schema

- `security-wasp-drone`: audit RLS policies; verify PII flags map to retention + encryption-at-rest config.
- `react-wasp-drone`: data-layer plan for the UI (TanStack Query / RSC / route loader).
- `mind-wasp-drone`: retrieval over `documents.embedding` (chunking, top-k, reranking).
- `quality-wasp-drone`: verification queries after first deploy.
