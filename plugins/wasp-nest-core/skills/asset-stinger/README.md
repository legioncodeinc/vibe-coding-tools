# asset-wasp-drone: Companion Resources

This directory holds everything the `asset-wasp-drone` agent needs to own the Universal Asset Registry. The pattern is generic: any product can adopt the registry; this Stinger describes the canonical 19-asset taxonomy, the registration workflow, the drift-audit mechanism, the sync-generator contract, and the deprecation/sunset rules. Organized into four layers: **guides** (workflow rules), **schema** (canonical Prisma + SQL), **examples** (exemplars to mirror), **templates** (seeds for kb + migrations).

> **Agent entry point:** [`.claude/agents/asset-wasp-drone.md`](../asset-wasp-drone.md) (repo-local). The agent reads files from this directory by path; it does not auto-load everything into context.
>
> **Peer wasp-drones:** [`library-wasp-drone`](../library-wasp-drone.md), [`quality-wasp-drone`](../quality-wasp-drone.md), [`security-wasp-drone`](../security-wasp-drone.md), [`ux-ui-svelte-wasp-drone`](../ux-ui-svelte-wasp-drone.md). Scope boundaries live in [`guides/05-hand-offs.md`](guides/05-hand-offs.md).
...
│   ├── 05-hand-offs.md                 # scope boundaries with other wasp-drones
│   └── assets/
...
| "write a QA report" | Hand off to [`quality-wasp-drone`](../quality-wasp-drone.md). |
| "write a PRD" | Draft; hand off to [`library-wasp-drone`](../library-wasp-drone.md) for numbering/invariants. |
...
1. **Parse intent** → match to one row in the Router table (see `asset-wasp-drone.md`).
...
6. **Produce the artifact** and report back with: what, where, what other wasp-drone (if any) should follow up.

## Supersession

This agent is new: it does not supersede any prior agent. It complements `library-wasp-drone` (which owns generic documentation) with registry-specific authority.
