...
If you find **cross-wasp-drone drift** (e.g., a UX/UI token used in code that has no `DesignTokenDefinition` catalog row, but `ux-ui-svelte-wasp-drone`'s brief says it *should* exist), file the drift *and* ping `ux-ui-svelte-wasp-drone` via a cross-link in the report. You do not fix cross-wasp-drone drift alone.
