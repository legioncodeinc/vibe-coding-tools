...
| `semanticRole` | `String?` | human | yes | human description: "primary brand color used on CTAs"; maintained by ux-ui-svelte-wasp-drone |
...
- **Required annotations:** none in code; the generator derives `category` from the prefix (`--color-*` to `color`, `--radius-*` to `radius`, etc.). `semanticRole` must be filled by ux-ui-svelte-wasp-drone via the admin UI.
...
- **ux-ui-svelte-wasp-drone** owns `semanticRole` entirely. You never invent what a token means.
...
- [ ] `semanticRole` is filled by ux-ui-svelte-wasp-drone (not by you)
- [ ] `tenantOverridable` defaults to `true` unless ux-ui-svelte-wasp-drone opts out
