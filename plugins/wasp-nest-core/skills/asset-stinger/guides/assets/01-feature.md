...
- **library-wasp-drone**: feature PRDs are numbered + placed at `library/requirements/<lifecycle>/prd-<###>-<title>/` + cross-linked by them.
- **ux-ui-svelte-wasp-drone**: every feature that renders UI must reference a surface/control/display catalogued by you, but *visually specified* by ux-ui.
- **security-wasp-drone**: any feature that touches auth, PII, or external integrations must route through a security review before `phase: shipped`.
- **quality-wasp-drone**: verifies the feature's PRD vs implementation; does NOT verify registry linkage (that's your drift audit).
