...
Standard. Control deprecation is usually tied to a variant rename or a broader design-system shift. Cross-check the `ux-ui-svelte-wasp-drone` brief before deprecating.
...
- **ux-ui-svelte-wasp-drone**: enforces "four button variants only" (primary / secondary / outline / ghost). If a `variants` array lists a fifth (e.g., `gradient`), flag.
- **security-wasp-drone**: controls that accept user input (`kind: input`, `select`, `slider`) review for input validation coverage during feature-PRD security review.
