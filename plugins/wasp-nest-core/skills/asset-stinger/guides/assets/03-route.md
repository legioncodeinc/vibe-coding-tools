...
- **security-wasp-drone**: every route with `externalContract: true` or `auth: public` gets a review.
- **library-wasp-drone**: route catalog feature PRDs get cross-linked.
- **ux-ui-svelte-wasp-drone**: not usually involved unless type=page.
