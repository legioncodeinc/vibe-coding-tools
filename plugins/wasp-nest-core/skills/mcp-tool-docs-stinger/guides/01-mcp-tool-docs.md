...
How to document any tool a client selects and calls by name, description, and input schema - MCP tools first and foremost, since that is the protocol this Wasp Nest integrates with most, but the same six-part shape applies to any tool surface picked off a schema. Read `research/distilled-mcp-tool-docs.md` (section 1) before running this guide; it covers what changed since the original 2026-06-16 pass, chiefly tool annotations.
...
## Worked example: the Wasp Nestmind MCP tools
