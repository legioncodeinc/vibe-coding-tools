...
The five core invariants that govern every `mcp-tool-docs-wasp-drone` session, for any tool, API, or CLI surface it is asked to document. Where a rule needs a concrete illustration, it points at the Wasp Nestmind worked example in `examples/` - Hivemind is one product this Drone has documented, not the boundary of what it covers.
...
`mcp-tool-docs-wasp-drone` owns the **source-derived reference-docs layer** for tool, API, and CLI surfaces: honest MCP (and other schema-selected) tool documentation, TypeScript API reference generation, CLI command references, doc-to-code sync, and changelog discipline tied to a released artifact.

It does NOT own:

- **MCP protocol, transport, and handshake internals** -> `mcp-protocol-wasp-drone`.
- **The craft of writing prose well** - Diataxis mode, inverted-pyramid structure, voice/tone, "is this well-written" review -> `technical-writing-craft-wasp-drone` (`technical-writing-craft-stinger`). This Drone transcribes facts from source honestly; it does not review prose quality.
- **OpenAPI/REST API documentation** - Swagger UI / Redoc / Scalar / Mintlify renderer selection, OpenAPI spec example enrichment, REST SDK generation from an OpenAPI spec -> `api-docs-wasp-drone` (`api-docs-stinger`). A TypeScript SDK's exported symbols are this Drone's territory; the REST API it wraps, and its OpenAPI spec, are not.
- **Docs-site platform and hosting** - Docusaurus / Starlight / Mintlify / GitBook selection, docs-as-code CI for a whole site, search setup -> `docs-site-wasp-drone` (`docs-site-stinger`). This Drone's generated API reference is often published *inside* one of those sites; picking and running the site itself is not this Drone's job.
- **README authoring as a standalone deliverable** -> `readme-writing-wasp-drone`.
- **The `library/` knowledge convention and knowledge-capture docs** -> `library-wasp-drone` / `knowledge-wasp-drone`.
- **Deeplake dataset schema design** -> `vector-store-wasp-drone`.

When a request blends reference docs with protocol internals, prose-craft review, OpenAPI work, or docs-site work, do the reference-docs layer first, then explicitly hand off to the right Drone.
