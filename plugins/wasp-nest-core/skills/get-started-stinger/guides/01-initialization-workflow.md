...
1. **Confirm the target.** Identify the repository root (the directory containing `.git/`, or the directory the user names). This repository procedure never writes outside it; the separate, consent-based home procedure is in `08-first-session-onboarding.md`.
...
6. **Document existing code.** If the repository already contains application code, run the `knowledge-stinger` playbook against that code. A `.git` directory alone is not evidence of an existing codebase.
7. **Mark completion last.** After the baseline and any required Knowledge pass succeed, write `wasp-nest.lock` with `scripts/mark-repo-started.py`, as described in guide `08-first-session-onboarding.md`. Do not write it for partial or declined setup.
