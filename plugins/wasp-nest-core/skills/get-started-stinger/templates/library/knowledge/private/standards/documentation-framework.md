...
Shared contract records live at `knowledge/private/contracts/CTR-<###>-<slug>.md`. A CTR defines one observable agreement used by independently owned PRDs, such as an API response, event payload, permission rule, or state transition. The contract-writing Drone owns the terms and acceptance. The Library Drone pins the accepted revision under `## Contract dependencies` in each affected PRD. A Draft or disputed CTR leaves that shared boundary unresolved; it does not permit dependent PRDs to be declared complete.
...
| Settle a boundary shared by separate PRDs | CTR under `knowledge/private/contracts/` |
