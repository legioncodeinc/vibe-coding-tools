...
This folder collects dependency audit reports produced by `dependency-audit-wasp-drone` for the `@deeplake/hivemind` package over time.
...
This folder is empty initially. The first report is created when `dependency-audit-wasp-drone` completes its first audit run against the package.
