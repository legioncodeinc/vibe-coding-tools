...
The canonical JSON shape every wiki-wasp-drone invocation returns to the graph driver. Mirrors [`guides/10-response-payload.md`](../guides/10-response-payload.md) - refer there for field semantics and driver-side reconciliation behavior. This file is the schema-only reference for tooling and validation.
...
Schema is the canonical contract between wiki-wasp-drone and Hivemind's graph driver (`src/graph/`). Field semantics in [`guides/10-response-payload.md`](../guides/10-response-payload.md). Validation patterns from `research/2026-04-29-frontmatter-validation.md` (Zod safeParse).
