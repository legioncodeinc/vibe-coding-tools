...
This directory holds everything the `wiki-wasp-drone` Drone needs to do its job. Organized into six layers: **guides** (procedural rules), **references** (cheat sheets loaded on demand), **templates** (page seeds copied per write), **examples** (worked invocations to mirror), **reports** (output shapes and past runs), **research** (audit trail for the guides).

> **Agent entry point:** [`.claude/agents/wiki-wasp-drone.md`](../../agents/wiki-wasp-drone.md) (repo-local). The agent reads files from this directory by path; it does not auto-load everything into context.
>
...
wiki-wasp-drone operates in four modes. The graph driver sets `mode` in the structured payload; for `@`-mention invocations infer the mode from user intent (and confirm with the user before writing per [`guides/02-direct-invocation.md`](guides/02-direct-invocation.md)).
...
- `library-wasp-drone` writes module narratives under `library/knowledge/private/<domain>/*.md`. wiki-wasp-drone does not touch that prose. See [`.claude/skills/library-stinger/README.md`](../library-stinger/README.md).
- `quality-wasp-drone` writes QA reports under `library/requirements/reports/` and feature/issue `reports/` folders. wiki-wasp-drone does not touch these.
- Hivemind's graph driver (`src/graph/`) owns `index.md`, `<type>/_index.md`, `log.md`, `hot.md`, `.hivemind/file-hashes.json` inside the knowledge area. wiki-wasp-drone writes per-page content only.
...
- Hot cache (`hot.md`) authorship - owned by the graph driver, not wiki-wasp-drone.
