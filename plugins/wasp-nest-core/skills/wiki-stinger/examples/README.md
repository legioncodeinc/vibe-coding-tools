...
Worked invocations of wiki-wasp-drone and the resulting page writes. Each file shows: the invocation payload, the source code chunk, the git context, and the resulting page writes (entity / concept / decision / contradiction-report as applicable).

Used by wiki-wasp-drone to mirror structure, tone, and frontmatter completeness when authoring real pages. All examples extract with tree-sitter and file pages into `library/knowledge/`.
