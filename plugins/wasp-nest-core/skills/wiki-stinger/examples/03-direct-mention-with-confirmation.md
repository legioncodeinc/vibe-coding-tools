...
A Cursor user is reading code in `src/mcp/server.ts` and `@`-mentions wiki-wasp-drone to extract entities for that file. Demonstrates the direct-invocation flow per [`guides/02-direct-invocation.md`](../guides/02-direct-invocation.md): self-discover the chunk, confirm scope before writing, fetch git context (driver-or-direct), respond with `partial_scan: true`.

## User prompt (in Cursor chat)

> @wiki-wasp-drone extract entities for the file I'm looking at

## Step 1 - Self-discover the chunk

Cursor's editor state shows `src/mcp/server.ts` is the active file. wiki-wasp-drone infers:
...
wiki-wasp-drone responds in Cursor chat:
...
wiki-wasp-drone writes to `.hivemind/queue/git-context-request-1745930400.json`:
...
wiki-wasp-drone summarizes for the user:
