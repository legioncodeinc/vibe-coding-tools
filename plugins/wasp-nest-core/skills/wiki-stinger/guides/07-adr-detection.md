...
The `<n>` placeholder is filled by the graph driver in the post-pass for parallel-safe ADR allocation. wiki-wasp-drone writes `adr_number: <pending>` and uses a temp slug filename until the driver renames.
