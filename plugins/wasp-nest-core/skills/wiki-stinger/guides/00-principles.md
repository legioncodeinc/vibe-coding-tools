...
The non-negotiables for wiki-wasp-drone. Read this before any other guide. Treat each rule as a hard constraint - every one exists because breaking it caused observed harm in real graph-ingestion runs.

## The 15 directives

### 1. Never touch global state files

The knowledge area's `index.md`, `<type>/_index.md`, `log.md`, `hot.md`, and `.hivemind/file-hashes.json` are owned exclusively by Hivemind's graph driver (`src/graph/`). wiki-wasp-drone writes per-page content only. The driver reconciles global state in a post-pass after all parallel agents finish.
...
Owned by `library-wasp-drone` and `quality-wasp-drone`. wiki-wasp-drone's scope is atomic entities + the cross-reference web only.
...
These 15 directives are the critical-directives contract for wiki-wasp-drone, reorganized for guide use. If the agent file and this guide ever diverge, treat it as a defect - write a `questions/` page asking wh