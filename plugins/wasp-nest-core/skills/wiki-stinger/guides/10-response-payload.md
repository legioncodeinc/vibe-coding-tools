...
Every wiki-wasp-drone invocation returns a structured JSON response payload to the graph driver. The driver's reconciliation pass depends on it. A scan that completes without a payload is a bug.
