...
In the linked git-config[1] documentation, the `mailmap.file` option gives the pathname of a mailmap file, and `mailmap.blob` gives a blob object (in the format described in this document) to read the mailmap from, relative to a repository's root, e.g. as an alternative to a `.mailmap` file when the repository is bare.
...
## Note on how commands apply mailmap

This page (the canonical gitmailmap reference on git-scm.com) documents only the `.mailmap` file format and location itself (SYNOPSIS, DESCRIPTION, SYNTAX, NOTES, EXAMPLES, reproduced above in full) and does not, on this page, separately narrate how individual commands such as `git log` or `git shortlog` consume it. In the git documentation set, the `--use-mailmap` / `--no-use-mailmap` (and the older `--mailmap`) command-line options, and the `log.mailmap` configuration variable, are documented on the git-log(1) and git-shortlog(1) man pages rather than repeated on this page; this page's DESCRIPTION paragraph is the source those pages point back to for the file format itself. No section describing git-log/git-shortlog behavior was cut from this fetch: the page's full content, top to bottom, is reproduced above.
