...
> Note
> You cannot verify email addresses from disposable email address services (services that allow you to receive email at a temporary address that expires after a certain time). If you'd like to keep your email address private, you can use a GitHub-provided `noreply` email address.
>
...
> Note (the noreply address forms)
...
This is the complete substantive content of the page, covering both noreply address forms (`USERNAME@users.noreply.github.com` for accounts predating July 18, 2017, and the ID-prefixed `ID+USERNAME@users.noreply.github.com` for accounts created after that date, or after re-toggling "Keep my email address private"), what each reveals or conceals, and the full command-line steps for both global and per-repository configuration. GitHub's navigation chrome and "In this article" links were dropped as boilerplate.
