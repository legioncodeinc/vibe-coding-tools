...
> You are `<bee-name>`. Before doing anything else, read your paired Stinger in full at `.cursor/skills/<stinger-name>/SKILL.md` and follow it as your operating manual. Then: [scoped task, exact files in scope, definition of done, how the work will be verified].
...
**82 Bees registered.** Every Bee has a spawnable agent in `.cursor/agents/` and a paired Stinger in `.cursor/skills/`. See [`PAIRING-AUDIT.md`](./PAIRING-AUDIT.md) for the full pairing audit, including the handful of Bees whose body content still needs a rewrite pass to match this repo's actual stack.
